    ...
    protected function getCapabilityResponse(string $scope): array
    {
        if ($scope === 'hosfin') {
            $summary = "สวัสดีค่ะ! หนูชื่อ **น้องมีตังค์** (RiMS AI) ผู้ช่วยสาวอัจฉริยะด้านการเงินการคลังประจำระบบ RiMS ยินดีให้บริการค่ะ 💰✨\n\n"
                ...
                . "1. **แผนเงินบำรุงโรงพยาบาล (PlanFin)**: ติดตามเป้าหมายแผนเงินบำรุง (Target vs Actual), รายได้-ค่าใช้จ่ายตามแผน, กำไรสุทธิ (Net Income), EBITDA และวงเงินลงทุน 20%\n"
                . "2. **เจ้าหนี้การค้า (AP)**: สรุปยอดหนี้แยกตามบริษัท/ผู้ขาย, รายการบิลค้างจ่าย, ใบรับวางบิล และประวัติการจ่ายเงิน\n"
                . "3. **ลูกหนี้ค่ารักษาพยาบาล (AR)**: ตรวจสอบลูกหนี้ค้างชำระแยกตามสิทธิ (UC, ประกันสังคม, ข้าราชการ) และลูกหนี้ตามผังบัญชี\n"
                . "4. **ผังบัญชีและงบทดลอง (Trial Balance)**: ขอยอดสรุปงบทดลองรายเดือน และยอดเดบิต/เครดิตแต่ละหมวด\n"
                . "5. **สมุดรายวันทั่วไป (Journal Voucher)**: ตรวจสอบการลงบัญชี GL รายวันและรายละเอียดเอกสารประกอบ\n"
                . "6. **เทียบเคียงระเบียบการเงิน (RAG)**: ระเบียบเงินบำรุงและแนวทางปฏิบัติการเงินการคลัง";

            $suggestions = [
                'สรุปเป้าหมายและผลดำเนินงานตามแผน PlanFin',
                'เปรียบเทียบรายได้จริงกับเป้าหมายตามแผนเงินบำรุง',
            ...
            $summary = "ในระบบ **HOSxP Setting** น้องมีตังค์สามารถช่วยท่านตรวจสอบความถูกต้องของข้อมูลพื้นฐานของโรงพยาบาลได้ครบทั้ง 8 หมวดค่ะ เช่น:\n\n"
                . "1. **แพทย์และบุคลากร (doctor)**: ตรวจสอบเลข ว., สภาวิชาชีพ, เลขบัตรประชาชน 13 หลัก\n"
                . "2. **ค่ารักษาพยาบาล (nondrugitems)**: ตรวจสอบการผูกรหัสมาตรฐาน ADP Code และหมวดรายได้\n"
                . "3. **ข้อมูลยา (drugitems)**: ตรวจสอบรหัสมาตรฐาน 24 หลัก (NDC 24 digits), รหัส TMT, บัญชียา ED/NED และราคาขาย\n"
                . "4. **ข้อมูลแล็บ (lab_items)**: ตรวจสอบการผูก icode คิดเงิน, รหัสมาตรฐาน TMLT และชุดตรวจ Profile\n"
                . "5. **สิทธิการรักษา (pttype & subinscl)**: ตรวจสอบรหัสส่งออก 4 หลัก, hipdata_code และสิทธิ สปสช.\n"
                . "6. **รหัสโรค ICD-10 (icd101)**: ตรวจสอบรหัสที่ปิดใช้งาน (active_status = N) และรหัสที่ สกส. ไม่รับเป็นโรคหลัก (accpdx = N)\n"
                . "7. **รหัสหัตถการ ICD-9 (icd9cm1)**: ตรวจสอบสถานะเปิด/ปิดใช้งาน, การส่งออกหัตถการ และสิทธิประกันสังคม SSS";

            $suggestions = [
                'ตรวจรหัสโรค ICD-10 ที่ สกส ไม่รับเป็นโรคหลัก',
                'ตรวจรายการยาที่ยังไม่ผูกรหัส 24 หลัก หรือ TMT',
                'ตรวจสอบรายการค่าบริการที่ยังไม่ผูกรหัส ADP',
                'ตรวจรายการแล็บที่ยังไม่ผูก icode คิดเงิน',
