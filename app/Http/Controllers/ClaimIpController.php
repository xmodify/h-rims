...
class ClaimIpController extends Controller
{
    ...
    public function ofc(Request $request)
    {
        ...
        $sum_month = DB::connection('hosxp')->select('
            SELECT month, COUNT(an) AS an, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total
            FROM (
                SELECT i.dchdate, i.an, a.income - a.rcpt_money AS claim_price, 
                ...
                LEFT JOIN ipt_pttype ip ON ip.an = i.an
                LEFT JOIN pttype p ON p.pttype = ip.pttype
                LEFT JOIN (SELECT an, SUM(receive_total) AS receive_total FROM hrims.stm_ofc GROUP BY an) stm ON stm.an = i.an
                LEFT JOIN (SELECT an, SUM(gtotal) AS gtotal FROM hrims.stm_ofc_cipn GROUP BY an) cipn ON cipn.an = i.an
                LEFT JOIN (SELECT d2.an, SUM(c.amount) AS amount FROM hrims.debtor_1102050101_402 d2 
                    JOIN hrims.stm_ofc_csop c ON c.hn = d2.hn AND c.vstdate BETWEEN d2.regdate AND d2.dchdate
                    GROUP BY d2.an) csop ON csop.an = i.an
                WHERE i.confirm_discharge = "Y" 
                    AND i.dchdate BETWEEN ? AND ? 
            ...
            GROUP BY month
            ORDER BY MIN(dchdate)', [$start_date_b, $end_date_b]);
        ...
        $search = DB::connection('hosxp')->select('
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                ...
                COALESCE(inc.income,0) - COALESCE(rc.rcpt_money,0) AS claim_price,kidney.kidney_list,ict.ipt_coll_status_type_name,
        ...
        $claim = DB::select('
            ...
            LEFT JOIN (SELECT d2.an,SUM(c.amount) AS amount,GROUP_CONCAT(c.rid) AS rid FROM hrims.debtor_1102050101_402 d2 
                JOIN hrims.stm_ofc_csop c ON c.hn = d2.hn AND c.vstdate BETWEEN d2.regdate AND d2.dchdate
    ...
    public function bkk(Request $request)
    {
        ...
        $sum_month = DB::connection('hosxp')->select('
            ...
            LEFT JOIN pttype p ON p.pttype=ip.pttype          
            LEFT JOIN an_stat a ON a.an=i.an            
            LEFT JOIN (SELECT an, SUM(receive_total) AS receive_total,GROUP_CONCAT(repno) AS repno FROM hrims.stm_ofc GROUP BY an) stm ON stm.an = i.an
            WHERE i.confirm_discharge = "Y" 
            AND i.dchdate BETWEEN ? AND ?
