...
class DebtorController extends Controller
{
    ...
    public function _1102050101_201_counts_ajax(Request $request)
    {
        $start_date = $request->start_date;
        $end_date = $request->end_date;

        $count_tab1 = \App\Models\Debtor_1102050101_201::whereBetween('vstdate', [$start_date, $end_date])->count();
    ...
    public function _1102050101_203_counts_ajax(Request $request)
    {
        $start_date = $request->start_date;
        $end_date = $request->end_date;

        $count_tab1 = \App\Models\Debtor_1102050101_203::whereBetween('vstdate', [$start_date, $end_date])->count();
    ...
    public function _1102050101_301_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        // Tab 1 count
        if ($search) {
            $tab1 = DB::table('debtor_1102050101_301')
                ->whereBetween('vstdate', [$start_date, $end_date])
                ->where(function($q) use ($search) {
                    $q->where('ptname', 'LIKE', "%{$search}%")
                      ->orWhere('hn', 'LIKE', "%{$search}%");
    ...
    public function _1102050101_303_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        // Tab 1 count
        if ($search) {
            $tab1 = DB::table('debtor_1102050101_303')
                ->whereBetween('vstdate', [$start_date, $end_date])
                ->where(function($q) use ($search) {
                    $q->where('ptname', 'LIKE', "%{$search}%")
                      ->orWhere('hn', 'LIKE', "%{$search}%");
    ...
    public function _1102050101_307_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        // Tab 1 count
        if ($search) {
            $tab1 = Debtor_1102050101_307::whereBetween(DB::raw('IFNULL(vstdate, dchdate)'), [$start_date, $end_date])
                ->where(function($q) use ($search) {
                    $q->where('ptname', 'LIKE', "%{$search}%")
                      ->orWhere('hn', 'LIKE', "%{$search}%")
                      ->orWhere('an', 'LIKE', "%{$search}%");
            ...
            $tab1 = Debtor_1102050101_307::whereBetween(DB::raw('IFNULL(vstdate, dchdate)'), [$start_date, $end_date])->count();
        }
        ...
        // Tab 3 count (IPD)
        $tab3Query = DB::connection('hosxp')->select('
            SELECT COUNT(DISTINCT i.an) AS c
            FROM ipt i
            LEFT JOIN ipt_pttype ip ON ip.an = i.an
            LEFT JOIN (SELECT o.an,o.pttype,SUM(o.sum_price) AS income
                FROM opitemrece o
            ...
            LEFT JOIN (SELECT r.vn AS an, SUM(r.total_amount) AS rcpt_money
                ...
                WHERE a.rcpno IS NULL 
                GROUP BY r.vn) rc ON rc.an = i.an
            LEFT JOIN (SELECT o.an,o.pttype,SUM(o.sum_price) AS other_price
                FROM opitemrece o
                INNER JOIN ipt i2 ON i2.an = o.an AND i2.dchdate BETWEEN ? AND ?
                LEFT JOIN hrims.lookup_icode li ON li.icode = o.icode WHERE li.kidney = "Y"
        ...
        ', [$start_date, $end_date, $start_date, $end_date, $start_date, $end_date]);
        $tab3 = $tab3Query[0]->c ?? 0;

        return response()->json(['tab1' => $tab1, 'tab2' => $tab2, 'tab3' => $tab3]);
    }
    ...
    public function _1102050101_309_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $pttype_sss_ae = DB::table('main_setting')->where('name', 'pttype_sss_ae')->value('value');
        ...
        if ($search) {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050101_309 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?', ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050101_309 WHERE vstdate BETWEEN ? AND ?', [$start_date, $end_date])[0]->cnt;
        }
        ...
        $tab2 = DB::connection('hosxp')->select('
            ...
            INNER JOIN (SELECT op.vn,SUM(op.sum_price) AS kidney_price
                FROM opitemrece op INNER JOIN hrims.lookup_icode li ON op.icode = li.icode AND li.kidney = "Y"
                WHERE op.vstdate BETWEEN ? AND ? GROUP BY op.vn) kid ON kid.vn = o.vn
        ...
        $tab3 = DB::connection('hosxp')->select('
            SELECT COUNT(DISTINCT o.vn) AS cnt
            FROM ovst o    
            LEFT JOIN visit_pttype vp ON vp.vn = o.vn
            LEFT JOIN pttype p ON p.pttype = vp.pttype
    ...
    public function _1102050101_401_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050101_401 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?", ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050101_401 WHERE vstdate BETWEEN ? AND ?", [$start_date, $end_date])[0]->cnt;
        }
    ...
    public function _1102050101_501_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050101_501 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?', ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050101_501 WHERE vstdate BETWEEN ? AND ?', [$start_date, $end_date])[0]->cnt;
        }
    ...
    public function _1102050102_108_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050102_108 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?', ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select('SELECT COUNT(*) AS cnt FROM debtor_1102050102_108 WHERE vstdate BETWEEN ? AND ?', [$start_date, $end_date])[0]->cnt;
        }
    ...
    public function _1102050102_110_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_110 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?", ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_110 WHERE vstdate BETWEEN ? AND ?", [$start_date, $end_date])[0]->cnt;
        }
        ...
        $tab2 = DB::connection('hosxp')->select('
            ...
            LEFT JOIN visit_pttype vp ON vp.vn = o.vn
            LEFT JOIN pttype p ON p.pttype = vp.pttype
            LEFT JOIN (SELECT op.vn,op.pttype,SUM(op.sum_price) AS income FROM opitemrece op WHERE op.vstdate BETWEEN ? AND ? GROUP BY op.vn, op.pttype) inc ON inc.vn = o.vn AND inc.pttype = vp.pttype
            LEFT JOIN (SELECT r.vn,SUM( r.total_amount ) AS rcpt_money FROM rcpt_print r LEFT JOIN rcpt_abort a ON a.rcpno = r.rcpno WHERE a.rcpno IS NULL GROUP BY r.vn) rc ON rc.vn = o.vn
            LEFT JOIN (SELECT op.vn,SUM(CASE WHEN li.ems = "Y" THEN op.sum_price ELSE 0 END) AS other_price FROM opitemrece op INNER JOIN hrims.lookup_icode li ON op.icode = li.icode WHERE op.vstdate BETWEEN ? AND ? GROUP BY op.vn) ch ON ch.vn = o.vn
            WHERE (o.an IS NULL OR o.an = "")
            AND o.vstdate BETWEEN ? AND ?
    ...
    public function _1102050102_602_counts_ajax(Request $request)
    {
        ...
        if ($search) {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_602 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?", ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_602 WHERE vstdate BETWEEN ? AND ?", [$start_date, $end_date])[0]->cnt;
        }
        ...
        $tab2 = DB::connection('hosxp')->select('
            ...
            LEFT JOIN (SELECT op.vn,op.pttype,SUM(op.sum_price) AS income FROM opitemrece op WHERE op.vstdate BETWEEN ? AND ? GROUP BY op.vn, op.pttype) inc ON inc.vn = o.vn AND inc.pttype = vp.pttype
            LEFT JOIN (SELECT r.vn,SUM( r.total_amount ) AS rcpt_money FROM rcpt_print r LEFT JOIN rcpt_abort a ON a.rcpno = r.rcpno WHERE a.rcpno IS NULL GROUP BY r.vn) rc ON rc.vn = o.vn
            LEFT JOIN (SELECT op.vn,SUM(op.sum_price) AS other_price FROM opitemrece op INNER JOIN hrims.lookup_icode li ON op.icode = li.icode AND li.ems = "Y" WHERE op.vstdate BETWEEN ? AND ? GROUP BY op.vn) ems ON ems.vn = o.vn
            WHERE (o.an IS NULL OR o.an = "")
            AND o.vstdate BETWEEN ? AND ?
    ...
    public function _1102050102_801_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_801 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?", ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_801 WHERE vstdate BETWEEN ? AND ?", [$start_date, $end_date])[0]->cnt;
        }
    ...
    public function _1102050102_803_counts_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $search = $request->search;
        ...
        if ($search) {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_803 WHERE (ptname LIKE ? OR hn LIKE ?) AND vstdate BETWEEN ? AND ?", ["%$search%", "%$search%", $start_date, $end_date])[0]->cnt;
        } else {
            $tab1 = DB::select("SELECT COUNT(*) AS cnt FROM debtor_1102050102_803 WHERE vstdate BETWEEN ? AND ?", [$start_date, $end_date])[0]->cnt;
        }
