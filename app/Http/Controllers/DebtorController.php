...
class DebtorController extends Controller
{
    ...
    public function _summary(Request $request)
    {
        ...
        $_1102050102_106 = DB::connection('hosxp')->select("
            SELECT COUNT(DISTINCT d.vn) AS anvn, SUM(d.debtor) AS debtor,
                SUM(IFNULL(d.receive,0) + IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0)) AS receive
            ...
            WHERE d.vstdate BETWEEN ? AND ? AND d.debtor > 0", [$start_date, $end_date]);
        ...
        $_1102050102_110 = DB::select('
            ...
            FROM (SELECT d.vn,d.debtor,IFNULL(d.receive,0)+IFNULL(stm.receive_total,0)+IFNULL(k.receive_total,0)+IFNULL(srt.receive_total,0)+IFNULL(csop.receive_total,0)+CASE WHEN d.kidney > 0 THEN IFNULL(hd.receive_total,0) ELSE 0 END AS receive
            ...
            LEFT JOIN (SELECT hn,DATE(datetimeadm) AS vstdate,SUM(receive_total) AS receive_total
                ...
                AND stm.vstdate = d.vstdate
            LEFT JOIN (SELECT hn,DATE(datetimeadm) AS vstdate,SUM(receive_total) AS receive_total
                FROM stm_bmt_kidney GROUP BY hn,DATE(datetimeadm)) k ON k.hn = d.hn AND k.vstdate = d.vstdate
            LEFT JOIN (SELECT hn,vstdate,SUM(receive_total) AS receive_total
                FROM stm_srt GROUP BY hn,vstdate) srt ON srt.hn = d.hn AND srt.vstdate = d.vstdate
            LEFT JOIN (SELECT hn,vstdate,LEFT(vsttime,5) AS vsttime,SUM(amount) AS receive_total
                FROM stm_ofc_csop WHERE sys <> "HD" GROUP BY hn,vstdate,LEFT(vsttime,5)) csop ON csop.hn = d.hn
                AND csop.vstdate = d.vstdate AND csop.vsttime = LEFT(d.vsttime,5)
            LEFT JOIN (SELECT hn,vstdate,SUM(amount) AS receive_total
        ...
        $_1102050102_803 = DB::select('
            ...
            LEFT JOIN (SELECT hn,DATE(datetimeadm) AS vstdate,SUM(receive_total) AS receive_total
                ...
                AND stm.vstdate = d.vstdate
            LEFT JOIN (SELECT hn,DATE(datetimeadm) AS vstdate,SUM(receive_total) AS receive_total
                FROM stm_bkk_kidney GROUP BY hn,DATE(datetimeadm)) k ON k.hn = d.hn AND k.vstdate = d.vstdate
            WHERE d.vstdate BETWEEN ? AND ?) a', [$start_date, $end_date]);
        ...
        $_1102050102_107 = DB::connection('hosxp')->select("
            ...
            WHERE d.dchdate BETWEEN ? AND ? AND d.debtor > 0", [$start_date, $end_date]);
        ...
        $_1102050102_111 = DB::select('
            SELECT COUNT(DISTINCT a.an) AS anvn,SUM(a.debtor) AS debtor,SUM(a.receive) AS receive
            FROM (SELECT d.an,d.debtor,IFNULL(d.receive,0)+IFNULL(stm.receive_total,0)+IFNULL(k.receive_total,0)+IFNULL(srt.receive_total,0)+IFNULL(cipn.receive_total,0) AS receive
            FROM debtor_1102050102_111 d
            LEFT JOIN (SELECT an,SUM(receive_total) AS receive_total
                FROM stm_bmt WHERE an IS NOT NULL AND an != "" GROUP BY an) stm ON stm.an = d.an
            LEFT JOIN (SELECT d2.an,SUM(c.receive_total) AS receive_total FROM debtor_1102050102_111 d2
                JOIN stm_bmt_kidney c ON c.hn = d2.hn AND c.datetimeadm BETWEEN d2.regdate AND d2.dchdate
                GROUP BY d2.an) k ON k.an = d.an
            LEFT JOIN (SELECT an,SUM(receive_total) AS receive_total
                FROM stm_srt WHERE an IS NOT NULL AND an != "" GROUP BY an) srt ON srt.an = d.an
            LEFT JOIN (SELECT an,SUM(gtotal) AS receive_total
                FROM stm_ofc_cipn GROUP BY an) cipn ON cipn.an = d.an
        ...
        $_1102050102_804 = DB::select('
            SELECT COUNT(DISTINCT a.an) AS anvn,SUM(a.debtor) AS debtor,SUM(a.receive) AS receive
            FROM (SELECT d.an,d.debtor,IFNULL(d.receive,0)+IFNULL(stm.receive_total,0)+IFNULL(k.receive_total,0) AS receive
            FROM debtor_1102050102_804 d
            LEFT JOIN (SELECT an,SUM(receive_total) AS receive_total
                FROM stm_bkk WHERE an IS NOT NULL AND an != "" GROUP BY an) stm ON stm.an = d.an
            LEFT JOIN (SELECT d2.an,SUM(c.receive_total) AS receive_total FROM debtor_1102050102_804 d2
                JOIN stm_bkk_kidney c ON c.hn = d2.hn AND c.datetimeadm BETWEEN d2.regdate AND d2.dchdate
                GROUP BY d2.an) k ON k.an = d.an
            WHERE d.dchdate BETWEEN ? AND ?) a', [$start_date, $end_date]);
    ...
    public function _1102050101_109_delete(Request $request)
    {
        ...
        $all_items = Debtor_1102050101_109::whereIn('vn', $checkbox)->get();
        $locked_items = $all_items->where('debtor_lock', 'Y')->count();
        $deletable_items = $all_items->where('debtor_lock', '!=', 'Y')->pluck('vn')->toArray();

        if (count($deletable_items) > 0) {
            Debtor_1102050101_109::whereIn('vn', $deletable_items)->delete();
        }
    ...
    public function _1102050101_216_confirm_kidney(Request $request)
    {
        ...
        $checkbox = $request->input('checkbox_kidney') ?: $request->input('checkbox');
        if (empty($checkbox) || !is_array($checkbox)) {
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
            ...
            return redirect()->back()->with('error', 'กรุณาเลือกรายการที่จะยืนยัน');
        }
        $checkbox_string = implode(",", $checkbox); // แปลงเป็น string สำหรับ SQL IN
    ...
    public function _1102050101_216_confirm_cr(Request $request)
    {
        ...
        $checkbox = $request->input('checkbox_cr') ?: $request->input('checkbox');
        if (empty($checkbox) || !is_array($checkbox)) {
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
    ...
    public function _1102050101_216_confirm_anywhere(Request $request)
    {
        ...
        $checkbox = $request->input('checkbox_anywhere') ?: $request->input('checkbox');
        if (empty($checkbox) || !is_array($checkbox)) {
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
        ...
        if ($search) {
            $debtor = DB::connection('hosxp')->select("
                SELECT d.vstdate,d.vsttime,d.hn,d.vn,d.ptname,d.mobile_phone_number, d.pttype,d.hospmain,
                    ...
                    d.charge_date,d.charge_no,d.charge,d.receive_date, d.receive_no,  
                    IFNULL(d.receive,0) + IFNULL(r.total_amount,0) AS receive,
                    IFNULL(r.total_amount,0) AS receive_system,
                    d.adj_inc, d.adj_dec, d.adj_note, d.adj_date,
                    d.repno, r.rcpno, r.total_amount,IFNULL(t.visit,0) AS visit,
                    CASE WHEN (IFNULL(d.receive,0) + IFNULL(r.total_amount,0)
            ...
            $debtor = DB::connection('hosxp')->select("
                SELECT d.vstdate,d.vsttime,d.hn,d.vn,d.cid,d.ptname,d.mobile_phone_number, d.pttype,d.hospmain,
                    ...
                    IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0) AS receive_system,
                    d.adj_inc, d.adj_dec, d.adj_note, d.adj_date,
                    d.repno, r.rcpno, r.total_amount,IFNULL(t.visit,0) AS visit,
                    CASE WHEN (IFNULL(d.receive,0) + IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0)
                    + IFNULL(d.adj_inc,0) - IFNULL(d.adj_dec,0) - IFNULL(d.debtor,0)) >= -0.05 THEN 0 ELSE DATEDIFF(CURDATE(), d.vstdate) END AS days
                FROM hrims.debtor_1102050102_106 d
                LEFT JOIN (SELECT r.vn, SUM(r.total_amount) AS total_amount,
                    GROUP_CONCAT(CONCAT(r.rcpno, '|', r.bill_date) ORDER BY r.rcpno) AS rcpno
    ...
    public function _1102050102_106_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::connection('hosxp')->select("
            ...
            LEFT JOIN (SELECT r.vn, SUM(r.total_amount) AS total_amount,
                ...
                WHERE a.rcpno IS NULL
                GROUP BY r.vn) r ON r.vn = d.vn
            WHERE d.vstdate BETWEEN ? AND ? AND d.debtor > 0
        ...
        if ($search) {
            $debtor = DB::connection('hosxp')->select("
                ...
                LEFT JOIN (SELECT vn, COUNT(vn) AS visit  FROM hrims.debtor_1102050102_106_tracking
                    GROUP BY vn) t ON t.vn = d.vn
                WHERE d.vstdate BETWEEN ? AND ?
                AND (d.ptname LIKE CONCAT('%', ?, '%') OR d.hn LIKE CONCAT('%', ?, '%') OR d.vn LIKE CONCAT('%', ?, '%'))
                ORDER BY d.vstdate", [$start_date, $end_date, $search, $search, $search]);
        } else {
            $debtor = DB::connection('hosxp')->select("
                SELECT d.vstdate,d.vsttime,d.hn,d.vn,d.cid,d.ptname,d.mobile_phone_number, d.pttype,d.hospmain,
                    d.pdx,d.income,d.paid_money,d.rcpt_money,d.debtor,d.receive AS receive_manual, d.repno AS repno_manual,d.debtor_lock, 
    ...
    public function _1102050102_106_tracking_print(Request $request, $tracking_id)
    {
        $tracking = DB::table('debtor_1102050102_106_tracking')->where('tracking_id', $tracking_id)->first();
        if (!$tracking) {
            abort(404, 'ไม่พบข้อมูลการติดตาม');
        }

        $debtor = DB::table('debtor_1102050102_106')->where('vn', $tracking->vn)->first();
        if (!$debtor) {
    ...
    //_1102050102_106_tracking_delete-------------------------------------------------------------------------------------------------
    public function _1102050102_106_tracking_delete(Request $request, $tracking_id)
    {
        Debtor_1102050102_106_tracking::where('tracking_id', $tracking_id)->delete();
        ...
        if ($search) {
            $debtor = DB::select("
                SELECT d.vn,d.vstdate,d.vsttime,d.hn,d.cid,d.ptname,d.hipdata_code, d.pttype,d.hospmain,d.pdx,
                    ...
                    (IFNULL(d.receive,0) + IFNULL(stm.receive_total,0) + IFNULL(kidney.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(csop.receive_total,0) + CASE WHEN d.kidney > 0 THEN IFNULL(hd.receive_total,0) ELSE 0 END) AS receive,
                    d.adj_inc, d.adj_dec, d.adj_date, d.adj_note,
                    IFNULL(su.receive_pp,0) AS receive_ppfs,d.status,d.repno,stm.repno AS repno_ofc,kidney.repno AS rid,srt.repno AS rid_srt,hd.rid AS rid_hd,d.debtor_lock,
                    CONCAT_WS(CHAR(44), stm.repno, kidney.repno, srt.repno, csop.round_no, hd.round_no) AS stm_round_no,
                    CONCAT_WS(CHAR(44), stm.receipt_date, kidney.receipt_date, srt.receipt_date, csop.receipt_date, hd.receipt_date) AS stm_receipt_date,
                    CONCAT_WS(CHAR(44), stm.receive_no, kidney.receive_no, srt.receive_no, csop.receive_no, hd.receive_no) AS stm_receive_no,
                    CASE WHEN (IFNULL(d.receive,0)+IFNULL(stm.receive_total,0)+ IFNULL(kidney.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(csop.receive_total,0) + CASE WHEN d.kidney > 0 THEN IFNULL(hd.receive_total,0) ELSE 0 END
                ...
                LEFT JOIN (SELECT hn, vstdate, LEFT(vsttime,5) AS vsttime, SUM(amount) AS receive_total,
                    GROUP_CONCAT(rid) AS rid, GROUP_CONCAT(round_no) AS round_no,
                    GROUP_CONCAT(receipt_date) AS receipt_date, GROUP_CONCAT(receive_no) AS receive_no
                    FROM stm_ofc_csop WHERE sys <> 'HD' GROUP BY hn, vstdate, LEFT(vsttime,5)) csop ON csop.hn = d.hn
                    AND csop.vstdate = d.vstdate AND csop.vsttime = LEFT(d.vsttime,5)
                LEFT JOIN (SELECT hn, vstdate, SUM(amount) AS receive_total,
    ...
    public function _1102050102_110_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::select("
            SELECT vstdate,COUNT(DISTINCT vn) AS anvn,SUM(debtor) AS debtor,
                SUM(IFNULL(receive,0)+IFNULL(receive_total,0)+IFNULL(k_receive_total,0)+IFNULL(srt_receive_total,0)+IFNULL(csop_receive_total,0)+CASE WHEN kidney > 0 THEN IFNULL(hd_receive_total,0) ELSE 0 END) AS receive
            FROM (SELECT d.vstdate,d.vn,d.kidney,d.debtor,d.receive,stm.receive_total,
                    k.receive_total AS k_receive_total, srt.receive_total AS srt_receive_total,
                    csop.receive_total AS csop_receive_total, hd.receive_total AS hd_receive_total
                FROM debtor_1102050102_110 d   
                LEFT JOIN (SELECT hn, vstdate, LEFT(vsttime,5) AS vsttime, SUM(receive_total) AS receive_total
                    FROM hrims.stm_bmt WHERE (an IS NULL OR an = '' OR an = '-') GROUP BY hn, vstdate, LEFT(vsttime,5)) stm ON stm.hn = d.hn
                    AND stm.vstdate = d.vstdate AND stm.vsttime = LEFT(d.vsttime,5)
                LEFT JOIN (SELECT hn, DATE(datetimeadm) AS vstdate, SUM(receive_total) AS receive_total
                    FROM hrims.stm_bmt_kidney GROUP BY hn, DATE(datetimeadm)) k ON k.hn = d.hn AND k.vstdate = d.vstdate
                LEFT JOIN (SELECT hn, vstdate, LEFT(vsttime,5) AS vsttime, SUM(receive_total) AS receive_total
                    FROM hrims.stm_srt GROUP BY hn, vstdate, LEFT(vsttime,5)) srt ON srt.hn = d.hn
                    AND srt.vstdate = d.vstdate AND srt.vsttime = LEFT(d.vsttime,5)
                LEFT JOIN (SELECT hn, vstdate, LEFT(vsttime,5) AS vsttime, SUM(amount) AS receive_total
                    FROM hrims.stm_ofc_csop WHERE sys <> 'HD' GROUP BY hn, vstdate, LEFT(vsttime,5)) csop ON csop.hn = d.hn
                    AND csop.vstdate = d.vstdate AND csop.vsttime = LEFT(d.vsttime,5)
                LEFT JOIN (SELECT hn, vstdate, SUM(amount) AS receive_total
                    FROM hrims.stm_ofc_csop WHERE sys = 'HD' GROUP BY hn, vstdate) hd ON hd.hn = d.hn
                    AND hd.vstdate = d.vstdate
                WHERE d.vstdate BETWEEN ? AND ?) a
            GROUP BY vstdate ORDER BY vstdate", [$start_date, $end_date]);
    ...
    public function _1102050102_803_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::select("
            ...
            FROM (SELECT d.vstdate,d.vn,d.kidney,d.debtor,d.receive,stm.receive_total,
                ...
                FROM debtor_1102050102_803 d   
                LEFT JOIN (SELECT hn, DATE(datetimeadm) AS vstdate, SUM(receive_total) AS receive_total
                    FROM hrims.stm_bkk WHERE (an IS NULL OR an = '' OR an = '-') GROUP BY hn, DATE(datetimeadm)) stm ON stm.hn = d.hn
                    AND stm.vstdate = d.vstdate
                LEFT JOIN (SELECT hn, DATE(datetimeadm) AS vstdate, SUM(receive_total) AS receive_total
                    FROM hrims.stm_bkk_kidney GROUP BY hn, DATE(datetimeadm)) k ON k.hn = d.hn AND k.vstdate = d.vstdate
                WHERE d.vstdate BETWEEN ? AND ?) a
            GROUP BY vstdate ORDER BY vstdate", [$start_date, $end_date,]);
    ...
    public function _1102050101_202(Request $request)
    {
        ...
        return view('debtor.1102050101_202', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }
    ...
    public function _1102050101_202_search_ajax(Request $request)
    {
        ...
        $data = DB::connection('hosxp')->select('
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw,
                COALESCE(orece.income,0) AS income,COALESCE(rc.rcpt_money,0) AS rcpt_money,COALESCE(orece.cr_price,0) AS other,
                COALESCE(orece.income,0)-COALESCE(rc.rcpt_money,0)-COALESCE(orece.cr_price,0) AS debtor,orece.cr_list AS other_list,
                ict.ipt_coll_status_type_name,i.data_ok,"ยืนยันลูกหนี้" AS status
            FROM ipt i
            LEFT JOIN patient pt ON pt.hn = i.hn
            LEFT JOIN ipt_pttype ip ON ip.an = i.an
            ...
            LEFT JOIN ipt_coll_status_type ict ON ict.ipt_coll_status_type_id = ic.ipt_coll_status_type_id
            LEFT JOIN (SELECT o.an,o.pttype,SUM(o.sum_price) AS income,
                SUM(CASE WHEN (li.uc_cr = "Y" OR li.kidney = "Y" OR n.nhso_adp_code IN ("S1801","S1802")) THEN o.sum_price ELSE 0 END) AS cr_price,
                GROUP_CONCAT(DISTINCT CASE WHEN (li.uc_cr = "Y" OR li.kidney = "Y" OR n.nhso_adp_code IN ("S1801","S1802")) THEN COALESCE(s.name, n.name) END) AS cr_list
                FROM opitemrece o
                INNER JOIN ipt i2 ON i2.an = o.an AND i2.confirm_discharge = "Y" AND i2.dchdate BETWEEN ? AND ?
                LEFT JOIN hrims.lookup_icode li ON li.icode = o.icode 
            ...
            GROUP BY i.an, ip.pttype
            ORDER BY i.ward, i.dchdate, i.an, ip.pttype', [$start_date, $end_date, $start_date, $end_date]);
    ...
    public function _1102050101_217(Request $request)
    {
        ...
        return view('debtor.1102050101_217', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }
    ...
    public function _1102050101_217_search_ajax(Request $request)
    {
        ...
        $debtor_search = DB::connection('hosxp')->select('
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw,
                IFNULL(orece.income,0) AS income,IFNULL(rc.rcpt_money,0) AS rcpt_money,
                IFNULL(orece.cr_price,0) AS cr,
                IFNULL(orece.cr_price,0) AS debtor,
                orece.cr_list,ict.ipt_coll_status_type_name,i.data_ok
            FROM ipt i
            LEFT JOIN patient pt ON pt.hn = i.hn
            LEFT JOIN ipt_pttype ip ON ip.an = i.an
            LEFT JOIN pttype p ON p.pttype = ip.pttype
            ...
            INNER JOIN (SELECT o.an, o.pttype, SUM(o.sum_price) AS income,
                SUM(CASE WHEN (li.uc_cr = "Y" OR li.kidney = "Y" OR n.nhso_adp_code IN ("S1801","S1802")) THEN o.sum_price ELSE 0 END) AS cr_price,
                GROUP_CONCAT(DISTINCT CASE WHEN (li.uc_cr = "Y" OR li.kidney = "Y" OR n.nhso_adp_code IN ("S1801","S1802")) THEN IFNULL(s.name, n.name) END) AS cr_list
                FROM opitemrece o
                INNER JOIN ipt i2 ON i2.an = o.an AND i2.confirm_discharge = "Y" AND i2.dchdate BETWEEN ? AND ?
                LEFT JOIN hrims.lookup_icode li ON li.icode = o.icode
                LEFT JOIN nondrugitems n ON n.icode = o.icode
    ...
    public function _1102050101_217_confirm(Request $request)
    {
        ...
        $params = array_merge([$start_date, $end_date, $start_date, $end_date], $checkbox); // รวม parameters
        ...
        $debtor = DB::connection('hosxp')->select('
            ...
            AND i.an IN (' . $checkbox_string . ') 
            GROUP BY i.an, ip.pttype
            ORDER BY i.ward, i.dchdate, i.an, ip.pttype', $params);
    ...
    public function _1102050101_302(Request $request)
    {
        ...
        return view('debtor.1102050101_302', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_302_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $pttype_sss_fund = DB::table('main_setting')->where('name', 'pttype_sss_fund')->value('value');
        ...
        $debtor_search = DB::connection('hosxp')->select('
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw, 
                IFNULL(orece.income,0) AS income,IFNULL(rc.rcpt_money,0) AS rcpt_money,IFNULL(orece.other_price,0) AS other,
                IFNULL(orece.income,0)-IFNULL(rc.rcpt_money,0)-IFNULL(orece.other_price,0) AS debtor,orece.other_list,
                ict.ipt_coll_status_type_name,i.data_ok,"ยืนยันลูกหนี้" AS status
            FROM ipt i
            LEFT JOIN patient pt ON pt.hn = i.hn
            LEFT JOIN ipt_pttype ip ON ip.an = i.an
    ...
    public function _1102050101_304(Request $request)
    {
        ...
        return view('debtor.1102050101_304', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_304_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $pttype_sss_fund = DB::table('main_setting')->where('name', 'pttype_sss_fund')->value('value');
    ...
    public function _1102050101_308(Request $request)
    {
        ...
        return view('debtor.1102050101_308', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_308_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        $pttype_sss_72 = DB::table('main_setting')->where('name', 'pttype_sss_72')->value('value');
    ...
    public function _1102050101_402(Request $request)
    {
        ...
        return view('debtor.1102050101_402', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_402_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
        ...
        $debtor_search = DB::connection('hosxp')->select('
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw,
                COALESCE(inc.income,0) AS income,COALESCE(rc.rcpt_money,0) AS rcpt_money,COALESCE(inc.kidney_price,0) AS kidney,
                COALESCE(inc.income,0) - COALESCE(rc.rcpt_money,0) AS debtor,inc.kidney_list,ict.ipt_coll_status_type_name,
    ...
    public function _1102050101_502(Request $request)
    {
        ...
        return view('debtor.1102050101_502', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_502_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
    ...
    public function _1102050101_504(Request $request)
    {
        ...
        return view('debtor.1102050101_504', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_504_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
    ...
    public function _1102050104_504_confirm(Request $request)
    {
        // Dummy method name just to avoid conflicts, we will rewrite the entire confirm function below
    }
    ...
    public function _1102050101_704(Request $request)
    {
        ...
        return view('debtor.1102050101_704', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050101_704_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
    ...
    public function _1102050104_704_confirm(Request $request)
    {
        // Dummy method
    }
    ...
    public function _1102050104_704_indiv_excel(Request $request)
    {
        // Dummy method
    }
    ...
    public function _1102050102_107(Request $request)
    {
        ...
        if ($search) {
            $debtor = DB::connection('hosxp')->select('
                SELECT d.*, d.receive AS receive_manual, d.repno AS repno_manual, 
                    ...
                    CASE WHEN (IFNULL(d.receive,0) + IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0)) + IFNULL(d.adj_inc,0) - IFNULL(d.adj_dec,0) - IFNULL(d.debtor,0) >= -0.05
                    THEN 0 ELSE DATEDIFF(CURDATE(), d.dchdate) END AS days
                FROM hrims.debtor_1102050102_107 d
                LEFT JOIN (SELECT r.vn, SUM(r.total_amount) AS total_amount,
                    GROUP_CONCAT(CONCAT(r.rcpno, '|', r.bill_date) ORDER BY r.rcpno) AS rcpno
        ...
        return view('debtor.1102050102_107', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'debtor_search_iclaim', 'count_tab1'));
    }
    ...
    public function _1102050102_107_search_ajax(Request $request)
    {
        ...
        $data = DB::connection('hosxp')->select('
            ...
            LEFT JOIN (SELECT r.vn AS an, SUM(r.total_amount) AS rcpt_money,
                GROUP_CONCAT(r.rcpno ORDER BY r.rcpno) AS rcpno 
                FROM rcpt_print r
                INNER JOIN ipt i3 ON i3.an = r.vn AND i3.dchdate BETWEEN ? AND ?
                LEFT JOIN rcpt_abort a ON a.rcpno = r.rcpno 
                WHERE a.rcpno IS NULL 
                GROUP BY r.vn) rc ON rc.an = i.an
            ...
            GROUP BY i.an
            ORDER BY i.dchdate', [$start_date, $end_date, $start_date, $end_date, $start_date, $end_date, $start_date, $end_date]);
    ...
    public function _1102050102_107_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::connection('hosxp')->select("
            ...
            LEFT JOIN (SELECT r.vn, SUM(r.total_amount) AS total_amount
                ...
                WHERE a.rcpno IS NULL
                GROUP BY r.vn) r ON r.vn = d.an
            WHERE d.dchdate BETWEEN ? AND ? AND d.debtor > 0
    ...
    public function _1102050102_109_search_ajax(Request $request)
    {
        ...
        $debtor_search = DB::connection('hosxp')->select(
            ...
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw, 
                COALESCE(total.income,0) AS income,a.income,COALESCE(rc.rcpt_money,0) AS rcpt_money,COALESCE(total.other_price,0) AS other,
                COALESCE(total.income,0)-COALESCE(rc.rcpt_money,0)-COALESCE(total.other_price,0) AS debtor,total.other_list,
                ict.ipt_coll_status_type_name,i.data_ok,"ยืนยันลูกหนี้" AS status
            FROM ipt i
            LEFT JOIN patient pt ON pt.hn = i.hn
    ...
    public function _1102050102_111(Request $request)
    {
        ...
        if ($search) {
            $debtor = DB::select('
                SELECT d.hn,d.an,d.cid,d.ptname,d.pttype,d.regdate,d.regtime,d.dchdate,d.dchtime,d.pdx,d.adjrw,
                    ...
                    d.charge_date,d.charge_no,d.charge,d.receive_date,d.receive_no,d.status,
                    d.receive AS receive_manual,d.repno AS repno_manual,d.adj_inc,d.adj_dec,d.adj_date,d.adj_note,
                    (IFNULL(d.receive,0) + IFNULL(stm.receive_total,0) + IFNULL(k.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(cipn.receive_total,0)) AS receive,
                    stm.repno, cipn.rid AS cipn_rid, k.repno AS csop_rid, srt.repno AS srt_rid,
                    CONCAT_WS(CHAR(44), stm.repno, k.repno, srt.repno, cipn.round_no) AS stm_round_no,
                    CONCAT_WS(CHAR(44), stm.receipt_date, k.receipt_date, srt.receipt_date, cipn.receipt_date) AS stm_receipt_date,
                    CONCAT_WS(CHAR(44), stm.receive_no, k.receive_no, srt.receive_no, cipn.receive_no) AS stm_receive_no,
                    CASE WHEN (IFNULL(d.receive,0) + IFNULL(stm.receive_total,0) + IFNULL(k.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(cipn.receive_total,0)
                    + IFNULL(d.adj_inc,0) - IFNULL(d.adj_dec,0) - IFNULL(d.debtor,0)) >= -0.01
                    THEN 0 ELSE DATEDIFF(CURDATE(), d.dchdate) END AS days
                FROM debtor_1102050102_111 d    
                LEFT JOIN (SELECT an, SUM(receive_total) AS receive_total, GROUP_CONCAT(repno) AS repno,
                ...
                LEFT JOIN (SELECT an, SUM(gtotal) AS receive_total, GROUP_CONCAT(rid) AS rid,
                    GROUP_CONCAT(round_no) AS round_no, GROUP_CONCAT(receipt_date) AS receipt_date, GROUP_CONCAT(receive_no) AS receive_no
                    FROM hrims.stm_ofc_cipn GROUP BY an) cipn ON cipn.an = d.an
                WHERE (d.ptname LIKE CONCAT("%", ?, "%") OR d.hn LIKE CONCAT("%", ?, "%") OR d.an LIKE CONCAT("%", ?, "%"))
                AND d.dchdate BETWEEN ? AND ?', [$search, $search, $search, $start_date, $end_date]);
        ...
        return view('debtor.1102050102_111', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050102_111_search_ajax(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d');
        $end_date = $request->end_date ?: date('Y-m-d');
    ...
    public function _1102050102_111_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::select('
            SELECT a.dchdate AS vstdate,COUNT(DISTINCT a.an) AS anvn,SUM(a.debtor) AS debtor,SUM(a.receive_total) AS receive
            FROM (SELECT d.dchdate,d.an, MAX(d.debtor) AS debtor, IFNULL(stm.receive_total,0)
                    + IFNULL(k.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(cipn.receive_total,0) AS receive_total
            FROM debtor_1102050102_111 d    
            LEFT JOIN (SELECT an, SUM(receive_total) AS receive_total
                    FROM stm_bmt WHERE an IS NOT NULL AND an != "" GROUP BY an) stm ON stm.an = d.an
            LEFT JOIN (SELECT d2.an, SUM(c.receive_total) AS receive_total FROM debtor_1102050102_111 d2
        ...
        if ($search) {
            $debtor = DB::select('
                SELECT d.hn,d.an,d.cid,d.ptname,d.pttype,d.regdate,d.regtime,d.dchdate,d.dchtime,d.pdx,d.adjrw,
                    ...
                    (IFNULL(d.receive,0) + IFNULL(stm.receive_total,0) + IFNULL(k.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(cipn.receive_total,0)) AS receive,
                    stm.repno, cipn.rid AS cipn_rid, k.repno AS csop_rid, srt.repno AS srt_rid,
                    CONCAT_WS(CHAR(44), stm.round_no, k.round_no, srt.round_no, cipn.round_no) AS stm_round_no,
                    CONCAT_WS(CHAR(44), stm.receipt_date, k.receipt_date, srt.receipt_date, cipn.receipt_date) AS stm_receipt_date,
                    CONCAT_WS(CHAR(44), stm.receive_no, k.receive_no, srt.receive_no, cipn.receive_no) AS stm_receive_no,
                    CASE WHEN (IFNULL(d.receive,0) + IFNULL(stm.receive_total,0) + IFNULL(k.receive_total,0) + IFNULL(srt.receive_total,0) + IFNULL(cipn.receive_total,0)
                    + IFNULL(d.adj_inc,0) - IFNULL(d.adj_dec,0) - IFNULL(d.debtor,0)) >= -0.01
    ...
    public function _1102050102_603(Request $request)
    {
        ...
        return view('debtor.1102050102_603', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050102_603_search_ajax(Request $request)
    {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        $debtor_search = DB::connection('hosxp')->select(
            ...
            SELECT w.name AS ward,i.hn,pt.cid,i.vn,i.an,CONCAT(pt.pname, pt.fname, " ", pt.lname) AS ptname,a.age_y,
                p.name AS pttype,p.hipdata_code,ip.hospmain,i.regdate,i.regtime,i.dchdate,i.dchtime,a.pdx,i.adjrw, 
                COALESCE(inc.income,0) AS income,a.income,COALESCE(rc.rcpt_money,0) AS rcpt_money,COALESCE(inc.other_price,0) AS other,
                COALESCE(inc.income,0)-COALESCE(rc.rcpt_money,0)-COALESCE(inc.other_price,0) AS debtor,inc.other_list,
                ict.ipt_coll_status_type_name,i.data_ok,"ยืนยันลูกหนี้" AS status
            FROM ipt i
            LEFT JOIN patient pt ON pt.hn = i.hn
            LEFT JOIN ipt_pttype ip ON ip.an = i.an
            ...
            LEFT JOIN (SELECT o.an,o.pttype,SUM(o.sum_price) AS income,
                       SUM(CASE WHEN li.kidney = "Y" THEN o.sum_price ELSE 0 END) AS other_price,
                       GROUP_CONCAT(DISTINCT CASE WHEN li.kidney = "Y" THEN s.name ELSE NULL END) AS other_list
                FROM opitemrece o
                INNER JOIN ipt i2 ON i2.an = o.an AND i2.confirm_discharge = "Y" AND i2.dchdate BETWEEN ? AND ?
                LEFT JOIN hrims.lookup_icode li ON li.icode = o.icode
                LEFT JOIN s_drugitems s ON s.icode = o.icode
    ...
    public function _1102050102_802(Request $request)
    {
        ...
        return view('debtor.1102050102_802', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050102_802_search_ajax(Request $request)
    {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
    ...
    public function _1102050102_804(Request $request)
    {
        ...
        return view('debtor.1102050102_804', compact('start_date', 'end_date', 'search', 'debtor', 'debtor_search', 'count_tab1'));
    }

    public function _1102050102_804_search_ajax(Request $request)
    {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
    ...
    public function _1102050102_804_daily_pdf(Request $request)
    {
        ...
        $debtor = DB::select('
            SELECT a.dchdate AS vstdate,COUNT(DISTINCT a.an) AS anvn,SUM(a.debtor) AS debtor,SUM(a.receive_total) AS receive
            FROM (SELECT d.dchdate,d.an, MAX(d.debtor) AS debtor, IFNULL(stm.receive_total,0)
                    + IFNULL(k.receive_total,0) AS receive_total
            FROM debtor_1102050102_804 d    
            LEFT JOIN (SELECT an, SUM(receive_total) AS receive_total
                    FROM hrims.stm_bkk WHERE an IS NOT NULL AND an != "" GROUP BY an) stm ON stm.an = d.an
            LEFT JOIN (SELECT d2.an, SUM(c.receive_total) AS receive_total FROM debtor_1102050102_804 d2
        ...
        switch ($code) {
            ...
            case '1102050102_110':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.' . $date_col, $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(s.receive_total,0)) AS receive_total, YEAR(d.{$date_col}) AS y, MONTH(d.{$date_col}) AS m
                            FROM stm_bmt s JOIN {$table_name} d ON s.hn = d.hn AND s.vstdate = d.vstdate AND LEFT(s.vsttime,5) = LEFT(d.vsttime,5)
                            WHERE (s.an IS NULL OR s.an = '') AND d.{$date_col} BETWEEN ? AND ? GROUP BY y, m
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.' . $date_col, $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(h.receive_total,0)) AS receive_total, YEAR(d.{$date_col}) AS y, MONTH(d.{$date_col}) AS m
                            FROM stm_bmt_kidney h JOIN {$table_name} d ON h.hn = d.hn AND DATE(h.datetimeadm) = d.vstdate
                            WHERE d.{$date_col} BETWEEN ? AND ? GROUP BY y, m
                        ) AS a GROUP BY y, m ORDER BY y, m";
            ...
            case '1102050102_803':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.' . $date_col, $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(s.receive_total,0)) AS receive_total, YEAR(d.{$date_col}) AS y, MONTH(d.{$date_col}) AS m
                            FROM stm_bkk s JOIN {$table_name} d ON s.hn = d.hn AND s.vstdate = d.vstdate AND LEFT(s.vsttime,5) = LEFT(d.vsttime,5)
                            WHERE (s.an IS NULL OR s.an = '') AND d.{$date_col} BETWEEN ? AND ? GROUP BY y, m
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.' . $date_col, $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(h.receive_total,0)) AS receive_total, YEAR(d.{$date_col}) AS y, MONTH(d.{$date_col}) AS m
                            FROM stm_bkk_kidney h JOIN {$table_name} d ON h.hn = d.hn AND DATE(h.datetimeadm) = d.vstdate
                            WHERE d.{$date_col} BETWEEN ? AND ? GROUP BY y, m
                        ) AS a GROUP BY y, m ORDER BY y, m";
                $params = [$start_date_b, $end_date_b, $start_date_b, $end_date_b, $start_date_b, $end_date_b];
                break;
            case '1102050102_106':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.vstdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0)) AS receive_total, YEAR(d.vstdate) AS y, MONTH(d.vstdate) AS m
                            FROM hrims.{$table_name} d
                            LEFT JOIN (
            ...
            case '1102050102_111':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.dchdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(stm.receive_total,0)) AS receive_total, YEAR(d.dchdate) AS y, MONTH(d.dchdate) AS m
                            FROM stm_bmt stm JOIN {$table_name} d ON stm.an = d.an
                            WHERE d.dchdate BETWEEN ? AND ? GROUP BY y, m
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.dchdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(kd.receive_total,0)) AS receive_total, YEAR(d.dchdate) AS y, MONTH(d.dchdate) AS m
                            FROM stm_bmt_kidney kd JOIN {$table_name} d ON kd.hn = d.hn AND kd.datetimeadm BETWEEN d.regdate AND d.dchdate
                            WHERE d.dchdate BETWEEN ? AND ? GROUP BY y, m
                        ) AS a GROUP BY y, m ORDER BY y, m";
                $params = [$start_date_b, $end_date_b, $start_date_b, $end_date_b, $start_date_b, $end_date_b];
                break;
            case '1102050102_804':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.dchdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(stm.receive_total,0)) AS receive_total, YEAR(d.dchdate) AS y, MONTH(d.dchdate) AS m
                            FROM stm_bkk stm JOIN {$table_name} d ON stm.an = d.an
                            WHERE d.dchdate BETWEEN ? AND ? GROUP BY y, m
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.dchdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(kd.receive_total,0)) AS receive_total, YEAR(d.dchdate) AS y, MONTH(d.dchdate) AS m
                            FROM stm_bkk_kidney kd JOIN {$table_name} d ON kd.hn = d.hn AND kd.datetimeadm BETWEEN d.regdate AND d.dchdate
                            WHERE d.dchdate BETWEEN ? AND ? GROUP BY y, m
                        ) AS a GROUP BY y, m ORDER BY y, m";
                $params = [$start_date_b, $end_date_b, $start_date_b, $end_date_b, $start_date_b, $end_date_b];
                break;
            case '1102050102_107':
                $sql = "SELECT month_name, SUM(claim_price) AS claim_price, SUM(receive_total) AS receive_total FROM (
                            ...
                            UNION ALL
                            SELECT " . str_replace($filter_date_col, 'd.dchdate', $month_case) . " AS month_name, 0 AS claim_price, SUM(IFNULL(r.total_amount,0) - IFNULL(d.rcpt_money,0)) AS receive_total, YEAR(d.dchdate) AS y, MONTH(d.dchdate) AS m
                            FROM hrims.{$table_name} d
                            LEFT JOIN (
                                SELECT r.vn, SUM(r.total_amount) AS total_amount
                                FROM rcpt_print r
