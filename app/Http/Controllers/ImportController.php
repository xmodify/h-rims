...
class ImportController extends Controller
{
    ...
    //statement_kidney_index---------------------------------------------------------------------------------------------
    public function statement_kidney_index(Request $request)
    {
        return view('import.statement_kidney_index');
    }
    ...
    public function stm_ucs_getChartData(Request $request)
    {
        ...
        $rawData = DB::table('stm_ucs')
            ...
            ->where('round_no', '<>', '')
            ->whereRaw('(CAST(LEFT(round_no, 2) AS UNSIGNED) + 2500 + IF(CAST(SUBSTRING(round_no, 3, 2) AS UNSIGNED) >= 10, 1, 0)) = ?', [$budget_year])
            ->groupBy('month_no')
            ->get()
            ->keyBy('month_no');
    ...
    public function stm_ucs_kidney_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
        ...
        $rawData = DB::table('stm_ucs_kidney')
            ->select(
                DB::raw('CAST(SUBSTRING(repno, 7, 2) AS UNSIGNED) as month_no'),
                DB::raw('SUM(receive_total) as total_receive')
            )
            ->whereNotNull('repno')
            ->where('repno', '<>', '')
            ->whereRaw('((CAST(SUBSTRING(repno, 5, 2) AS UNSIGNED) + 2500) + IF(CAST(SUBSTRING(repno, 7, 2) AS UNSIGNED) >= 10, 1, 0)) = ?', [$budget_year])
            ->groupBy('month_no')
            ->get()
            ->keyBy('month_no');
    ...
    public function stm_bkk_kidney_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
        ...
        $rawData = DB::table('stm_bkk_kidney')
            ->select(
                DB::raw('CAST(SUBSTRING(repno, 13, 2) AS UNSIGNED) as month_no'),
                DB::raw('SUM(receive_total) as total_receive')
            )
            ->whereNotNull('repno')
            ->where('repno', '<>', '')
            ->whereRaw('((CAST(SUBSTRING(repno, 9, 4) AS UNSIGNED) + 543) + IF(CAST(SUBSTRING(repno, 13, 2) AS UNSIGNED) >= 10, 1, 0)) = ?', [$budget_year])
            ->groupBy('month_no')
            ->get()
            ->keyBy('month_no');
    ...
    public function stm_bmt_kidney_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
        ...
        $rawData = DB::table('stm_bmt_kidney')
            ->select(
                DB::raw('CAST(SUBSTRING(repno, 12, 2) AS UNSIGNED) as month_no'),
                DB::raw('SUM(receive_total) as total_receive')
            )
            ->whereNotNull('repno')
            ->where('repno', '<>', '')
            ->whereRaw('((CAST(SUBSTRING(repno, 8, 4) AS UNSIGNED) + 543) + IF(CAST(SUBSTRING(repno, 12, 2) AS UNSIGNED) >= 10, 1, 0)) = ?', [$budget_year])
            ->groupBy('month_no')
            ->get()
            ->keyBy('month_no');
    ...
    public function stm_ofc_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
    ...
    public function stm_bkk_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
    ...
    //stm_srt-----------------------------------------------------------------------------------------------------------------------------
    public function stm_srt(Request $request)
    {
        ini_set('max_execution_time', 300);
    ...
    public function stm_srt_save(Request $request)
    {
        ...
        try {
            ...
            $stm_srtexcel = Stm_srtexcel::whereNotNull('charge')
                ->where('charge', '<>', 'เรียกเก็บ')
                ->get();

            foreach ($stm_srtexcel as $value) {
                $exists = Stm_srt::where('repno', $value->repno)
                    ->where('no', $value->no)
                    ->exists();

                if ($exists) {
                    Stm_srt::where('repno', $value->repno)
                        ->where('no', $value->no)
                        ->update([
                            'round_no' => $value->round_no,
                            'datetimeadm' => $value->datetimeadm,
    ...
    //stm_srt_updateReceipt------------------------------------------------------------------------------------------------------------- 
    public function stm_srt_updateReceipt(Request $request)
    {
        $request->validate([
            'round_no' => 'required',
            'receive_no' => 'required|max:20',
    ...
    //stm_srt_detail----------------------------------------------------------------------------------------------------------------
    public function stm_srt_detail(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_srt_' . ($type == 'opd' ? 'OPD' : 'IPD') . '_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_srt_detail', compact('start_date', 'end_date'));
    }

    public function stm_srt_detail_opd(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_srt_OPD_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_srt_detail_opd', compact('start_date', 'end_date'));
    }

    public function stm_srt_detail_ipd(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_srt_IPD_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_srt_detail_ipd', compact('start_date', 'end_date'));
    }

    //stm_pvt-----------------------------------------------------------------------------------------------------------------------------
    public function stm_pvt(Request $request)
    {
        ini_set('max_execution_time', 300);
    ...
    //stm_pvt_save---------------------------------------------------------------------------------------------------------------------------
    public function stm_pvt_save(Request $request)
    {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        try {
            ...
            $stm_pvtexcel = Stm_pvtexcel::whereNotNull('charge')
                ->where('charge', '<>', 'เรียกเก็บ')
                ->get();

            foreach ($stm_pvtexcel as $value) {
                $exists = Stm_pvt::where('repno', $value->repno)
                    ->where('no', $value->no)
                    ->exists();

                if ($exists) {
                    Stm_pvt::where('repno', $value->repno)
                        ->where('no', $value->no)
                        ->update([
                            'round_no' => $value->round_no,
                            'datetimeadm' => $value->datetimeadm,
    ...
    //stm_pvt_updateReceipt------------------------------------------------------------------------------------------------------------- 
    public function stm_pvt_updateReceipt(Request $request)
    {
        $request->validate([
            'round_no' => 'required',
            'receive_no' => 'required|max:20',
    ...
    //stm_pvt_detail----------------------------------------------------------------------------------------------------------------
    public function stm_pvt_detail(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_pvt_' . ($type == 'opd' ? 'OPD' : 'IPD') . '_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_pvt_detail', compact('start_date', 'end_date'));
    }

    public function stm_pvt_detail_opd(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_pvt_OPD_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_pvt_detail_opd', compact('start_date', 'end_date'));
    }

    public function stm_pvt_detail_ipd(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d', strtotime("last day of this month"));
        ...
        if ($request->ajax() || $request->export == 'excel') {
            ...
            if ($request->export == 'excel') {
                ...
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment; filename="stm_pvt_IPD_' . date('YmdHis') . '.xlsx"');
                $writer->save('php://output');
                exit;
            }
        ...
        return view('import.stm_pvt_detail_ipd', compact('start_date', 'end_date'));
    }
    ...
    public function stm_bmt_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
    ...
    public function stm_ofc_csopdetail(Request $request)
    {
        ...
        // Export Excel (Original Logic with Filter)
        if ($request->export == 'excel') {
            $queryExport = DB::table('stm_ofc_csop')
                ->whereBetween('vstdate', [$start_date, $end_date]);

            if ($sys_type == 'csop') {
                $queryExport->where('sys', '<>', 'HD');
            } elseif ($sys_type == 'kidney') {
                $queryExport->where('sys', '=', 'HD');
            }

            $data = $queryExport->orderBy('station')->orderBy('round_no')->get();
        ...
        return view('import.stm_ofc_csopdetail', compact('start_date', 'end_date', 'sys_type'));
    }
    ...
    public function stm_lgo_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
    ...
    public function stm_sss_kidney_getChartData(Request $request)
    {
        $budget_year = $request->budget_year ?: DB::table('budget_year')
            ->whereDate('DATE_END', '>=', date('Y-m-d'))
            ->whereDate('DATE_BEGIN', '<=', date('Y-m-d'))
        ...
        $rawData = DB::table('stm_sss_kidney')
            ->select(
                DB::raw('CAST(SUBSTRING(RIGHT(round_no, 8), 5, 2) AS UNSIGNED) as month_no'),
                DB::raw('SUM(amount) + SUM(epopay) + SUM(epoadm) as total_receive')
            )
            ->whereNotNull('round_no')
            ->where('round_no', '<>', '')
    ...
    public function stm_sss_kidney_save(Request $request)
    {
        ...
        try {
            ...
            return redirect()
                ...
                ->with('stm_success', implode(', ', $docNames));
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    private function clean_excel_file($path)
    {
        $content = file_get_contents($path);
        if (str_contains($content, '<!DOCTYPE') || str_contains($content, '<!ENTITY')) {
            // Remove DOCTYPE and ENTITY to avoid XXE protection trigger in PhpSpreadsheet
            $content = preg_replace('/<!DOCTYPE.*?>/s', '', $content);
            $content = preg_replace('/<!ENTITY.*?>/s', '', $content);
