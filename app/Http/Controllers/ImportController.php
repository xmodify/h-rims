...
class ImportController extends Controller
{
    ...
    // API: Check for duplicate STM files from Extension
    public function api_stm_check(Request $request)
    {
        try {
            $type = $request->stm_type ?? 'unknown';
            $files = $request->files_scanned ?? []; // Array of {stm_no, date, url, ...}

            if (empty($files) || !is_array($files)) {
                return response()->json(['success' => false, 'message' => 'ไม่มีข้อมูลไฟล์ส่งมา'], 400);
            }

            // Map type to database column convention
            // Most tables store the filename in 'stm_filename' or similar. 
            // In stm_ucs, it's 'stm_filename'.
            $tableName = 'stm_ucs';
            if ($type === 'ofc') $tableName = 'stm_ofc';
            else if ($type === 'bmt') $tableName = 'stm_bmt'; // Need to verify if this exists
            else if ($type === 'bkk') $tableName = 'stm_ktmn'; // Need to verify
            else if ($type === 'srt') $tableName = 'stm_srt'; // Need to verify

            // Fast way: grab all existing filenames that match the batch (using like or direct match)
            // But STM files often have .xls extension in DB. Let's get all stm filenames currently in DB for these prefixes
            $fileNumbers = array_column($files, 'stm_no');

            // To be safe and fast, we just query if the string exists in the stm_filename column
            $existingFilesDb = DB::table($tableName)
                ->where(function ($query) use ($fileNumbers) {
                    foreach ($fileNumbers as $no) {
                        $query->orWhere('stm_filename', 'like', "%{$no}%");
                    }
                })
                ->select('stm_filename')
                ->distinct()
            ...
            // Check each file against existing
            $resultFiles = [];
            foreach ($files as $f) {
                // A file exists if any DB filename contains its stm_no
                $isExist = false;
                foreach ($existingFilesDb as $dbName) {
                    if (strpos($dbName, $f['stm_no']) !== false) {
                        $isExist = true;
                        break;
                    }
                }
                $f['status'] = $isExist ? 'exist' : 'new';
                $resultFiles[] = $f;
            }
    ...
    // API: Upload an STM file from Extension
    public function api_stm_upload(Request $request, $type)
    {
        Log::info("API STM Upload initiated", ['type' => $type, 'stm_no' => $request->stm_no]);
        try {
            if (!$request->hasFile('file')) {
                throw new \Exception("ไม่พบไฟล์แนบมาใน Request");
            }

            $request->validate([
                'file' => 'required|file',
                'stm_no' => 'required|string',
            ]);

            $uploadExt = strtolower($request->file('file')->getClientOriginalExtension());
            if ($uploadExt && !in_array($uploadExt, ['xls', 'xlsx', 'bin'])) {
                Log::warning("File extension invalid: " . $uploadExt);
                // allow empty or xls/xlsx
            }
            Log::info("Request validated successfully.");

            $file = $request->file('file');
            $originalName = $file->getClientOriginalName();
            Log::info("File received: {$originalName}, Size: " . $file->getSize());

            // Force the name to the stm_no if somehow it mismatched, but usually keeping original is fine.
            // Actually it's safer to use the provided name but ensure it ends in .xls
            $fileName = $request->stm_no . '.xls';
            Log::info("Target filename set to: {$fileName}");

            // Store it temporarily so the existing logic can read it
            // Using a specific temp path for API uploads
            $tempPath = storage_path('app/temp_stm_api');
            ...
            Log::info("File moved to: {$fullFilePath}");

            // Now we invoke the specific processing logic based on type.
            // This is essentially doing what `stm_ucs_save` does, but for a single file from a path

            if ($type === 'ucs') {
                Log::info("Calling process_ucs_file_api for UCS type.");
                $this->process_ucs_file_api($fullFilePath, $fileName);
                Log::info("Processed UCS file API successfully: {$fileName}");
            } else if ($type === 'ofc') {
                // $this->process_ofc_file_api($fullFilePath, $fileName);
                throw new \Exception("ระบบอัปโหลดสำหรับ OFC ยังไม่เปิดใช้งานแบบ API");
            } else {
                throw new \Exception("ไม่รองรับประเภท STM: {$type}");
            }

            // Cleanup
            if (File::exists($fullFilePath)) {
                File::delete($fullFilePath);
                Log::info("Cleaned up temporary file: {$fullFilePath}");
            }

            return response()->json([
                'success' => true,
                'message' => 'นำเข้าข้อมูล ' . $fileName . ' สำเร็จ'
            ]);
        } catch (\Exception $e) {
            Log::error("API STM Upload Error in {$type}: " . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'success' => false,
    ...
    // Helper: Process single UCS file (extracted core logic from stm_ucs_save)
    private function process_ucs_file_api($filePath, $fileName)
    {
        // 1) Read Excel
        ...
        foreach ([2, 3] as $sheetIndex) {
            ...
            for ($row = $startRow; $row <= $row_limit; $row++) {
                ...
                if (count($buffer) === 1000) {
                    $allBuffer = array_merge($allBuffer, $buffer);
                    $buffer = [];
                }
            }
        ...
        // 2) Insert to DB
        // Because it's an API call, we do it inline directly to stm_ucs to avoid staging table conflicts if multiple uploads happen at once
        DB::transaction(function () use ($allBuffer) {
            foreach ($allBuffer as $value) {
                // using updateOrInsert directly
