...
class ImportController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = DB::table('sss_equipdev_aipn');
            ...
            $recordsTotal = DB::table('sss_equipdev_aipn')->count();
            $recordsFiltered = $query->count();
        ...
        $total_records = DB::table('sss_equipdev_aipn')->count();
        $active_records = DB::table('sss_equipdev_aipn')->where('dateexp', '>=', now()->format('Y-m-d'))->count();
        $expired_records = DB::table('sss_equipdev_aipn')->where('dateexp', '<', now()->format('Y-m-d'))->count();
    ...
    public function sss_equipdev_aipn_save(Request $request)
    {
        ...
        try {
            ...
            for ($row = 2; $row <= $row_limit; $row++) {
                ...
                // Clean rates (e.g. remove commas or handles '-')
            ...
            if (!empty($data)) {
                \App\Models\SssEquipdevAipn::truncate();
                DB::transaction(function () use ($data) {
                    $chunks = array_chunk($data, 1000);
                    foreach ($chunks as $chunk) {
                        \App\Models\SssEquipdevAipn::insert($chunk);
                    }
                });
            }
