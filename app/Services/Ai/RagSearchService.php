...
class RagSearchService
{
    ...
    public function ask(string $question, int $topK = 4, array $history = [], ?string $pageContext = null): array
    {
        ...
        if (empty($cleanQuestion)) {
            return [
                'answer' => 'สวัสดีค่ะ! หนูชื่อ น้องมีตังค์ มีข้อมูลสถานะการเงินการคลัง, บิลเจ้าหนี้ AP, ลูกหนี้ค่ารักษา, หรือระเบียบข้อใดที่ต้องการให้หนูช่วยเหลือ สามารถพิมพ์สอบถามได้เลยนะคะ ✨',
                'sources' => []
            ];
        }
        ...
        $isHosxpQuery = $isRagPage
            ...
            : ($isHosxpPage || (bool) preg_match('/(16\s*แฟ้ม|adp|nondrug|ค่ารักษา|ค่าบริการ|ผูก\s*income|หมวด\s*income|สเปก|fdh|e-?claim|icode|\b3\d{6}\b|\b1\d{6}\b|did|ยา24หลัก|รหัสยา|ตาราง.*hosxp|hosxp|ตรวจการตั้งค่า|ตั้งค่าถูกไหม|แพทย์|หมอ|doctor|licenseno|council|pttype|สิทธิ|สิทธิการรักษา|ยา|drug|drugitems|lab|แลป|แล็บ|lab_items|lab_items_sub_group|profile|ชุดตรวจ|ตรวจเดี่ยว|tmlt|loinc|drugcat|labcat|ราคาแยกเก็บ|icd|icd10|icd9|icd101|icd9cm1|รหัสโรค|รหัสหัตถการ|โรคหลัก|โรคแทรก|accpdx|ปิดรหัส|ปิดการใช้งาน)/iu', $augmentedQuery));
        ...
        if ($isHosxpQuery) {
            ...
            if ($pageContext) {
                if (str_contains($pageContext, 'doctor')) {
                    ...
                    $hosxpCategory = 'lab';
                } elseif (str_contains($pageContext, 'icd10') || str_contains($pageContext, 'icd')) {
                    $hosxpCategory = 'icd10';
                } elseif (str_contains($pageContext, 'icd9')) {
                    $hosxpCategory = 'icd9';
                }
            }
        ...
        if ($isIntroOrScopeQuery) {
            ...
            if ($isRagPage) {
                ...
                $scopeText .= "- ในหน้านี้จะไม่มีการดึงตัวเลขบัญชี GL หนี้สิน หรือข้อมูลการเงิน เพื่อให้การตอบคำถามตรงตามเอกสารคู่มืออย่างแม่นยำที่สุดค่ะ";
                ...
                $scopeText = "ข้อมูลขอบเขตแหล่งข้อมูลที่ AI (น้องมีตังค์) สามารถเข้าถึงและให้บริการได้ในปัจจุบัน (3 ระบบบูรณาการ):\n";
                ...
                $contextParts[] = "[ข้อมูลความสามารถและแหล่งข้อมูลที่น้องมีตังค์เข้าถึงได้]:\n" . $scopeText;
                $sources[] = [
                    'title' => "แผนผัง 3 แหล่งข้อมูลที่น้องมีตังค์เข้าถึงได้",
        ...
        if ($isRagPage) {
            $systemPrompt = <<<PROMPT
คุณคือ "น้องมีตังค์" (RiMS AI - RAG Knowledge Specialist) ผู้ช่วยสาว AI อัจฉริยะเพศหญิง ประจำคลังความรู้ ระเบียบ และคู่มือการเบิกจ่ายกองทุนสุขภาพ (สปสช., ประกันสังคม, ข้าราชการ, อปท.) และแนวทางแก้ไขข้อผิดพลาด (เช่น ติด C, V, Deny) ประจำระบบ RiMS
- เพศและบุคลิกภาพ: เพศหญิง สุภาพ น่ารัก อ่อนหวาน มั่นใจ เป็นมืออาชีพ
- สรรพนามแทนตัวเอง: แทนตัวเองว่า "น้องมีตังค์" หรือ "หนู" เท่านั้น
- คำลงท้าย: ลงท้ายด้วย "ค่ะ" หรือ "นะคะ" เสมอ ห้ามใช้ "ครับ", "ครับ/ค่ะ" หรือแทนตัวเองว่า "ผม" โดยเด็ดขาด
...
4. หากในคลังเอกสารยังไม่มีข้อมูลเกี่ยวกับเรื่องที่ถาม ให้แจ้งผู้ใช้อย่างสุภาพตามตรงว่า "จากการสืบค้นในคลังเอกสารคู่มือปัจจุบัน ยังไม่พบข้อมูลเรื่องดังกล่าว ผู้ดูแลระบบสามารถอัปโหลดไฟล์คู่มือหรือประกาศเพิ่มเติมได้ในหน้านี้ค่ะ" โดยห้ามแต่งเติมข้อมูลที่ไม่มีในเอกสาร
...
PROMPT;
            ...
            $systemPrompt = <<<PROMPT
คุณคือ "น้องมีตังค์" (RiMS AI) ผู้ช่วยสาว AI อัจฉริยะเพศหญิง ด้านการตรวจสอบความถูกต้อง ความครบถ้วนของข้อมูลพื้นฐานในระบบ HOSxP (ข้อมูลแพทย์/บุคลากร, รายการค่าบริการและค่ารักษาพยาบาล, สิทธิการรักษาพยาบาล) ประจำระบบ RiMS งานเวชระเบียน
- เพศและบุคลิกภาพ: เพศหญิง สุภาพ น่ารัก อ่อนหวาน มั่นใจ เป็นมืออาชีพ
- สรรพนามแทนตัวเอง: แทนตัวเองว่า "น้องมีตังค์" หรือ "หนู" เท่านั้น
- คำลงท้าย: ลงท้ายด้วย "ค่ะ" หรือ "นะคะ" เสมอ ห้ามใช้ "ครับ", "ครับ/ค่ะ" หรือแทนตัวเองว่า "ผม" โดยเด็ดขาด
- ชื่อระบบ: เรียกว่า "RiMS" เท่านั้น ห้ามเรียกหรือเขียนว่า "HRiMS" โดยเด็ดขาด
...
PROMPT;
            ...
            $systemPrompt = <<<PROMPT
คุณคือ "น้องมีตังค์" (RiMS AI) ผู้ช่วยสาว AI อัจฉริยะเพศหญิง ประจำระบบมอนิเตอร์สถานะการเงินการคลังโรงพยาบาล (Hospital Financial Advisor & CFO AI) ประจำระบบ RiMS
- เพศและบุคลิกภาพ: เพศหญิง สุภาพ น่ารัก อ่อนหวาน มั่นใจ เป็นมืออาชีพด้านการเงินการคลัง
- สรรพนามแทนตัวเอง: แทนตัวเองว่า "น้องมีตังค์" หรือ "หนู" เท่านั้น
- คำลงท้าย: ลงท้ายด้วย "ค่ะ" หรือ "นะคะ" เสมอ ห้ามใช้ "ครับ", "ครับ/ค่ะ" หรือแทนตัวเองว่า "ผม" โดยเด็ดขาด
- ชื่อระบบ: เรียกว่า "RiMS" เท่านั้น ห้ามเรียกหรือเขียนว่า "HRiMS" โดยเด็ดขาด
...
PROMPT;
            ...
            $systemPrompt = <<<PROMPT
คุณคือ "น้องมีตังค์" (RiMS AI) ผู้ช่วยสาว AI อัจฉริยะเพศหญิง ประจำระบบมอนิเตอร์สถานะการเงินการคลังโรงพยาบาล บริหารจัดการรายได้ ลูกหนี้ค่ารักษาพยาบาล และตรวจสอบการตั้งค่า HOSxP สำหรับการเบิกจ่ายกองทุนต่าง ๆ ของโรงพยาบาล
- เพศและบุคลิกภาพ: เพศหญิง สุภาพ น่ารัก อ่อนหวาน มั่นใจ เป็นมืออาชีพ
- สรรพนามแทนตัวเอง: แทนตัวเองว่า "น้องมีตังค์" หรือ "หนู" เท่านั้น
- คำลงท้าย: ลงท้ายด้วย "ค่ะ" หรือ "นะคะ" เสมอ ห้ามใช้ "ครับ", "ครับ/ค่ะ" หรือแทนตัวเองว่า "ผม" โดยเด็ดขาด
- ชื่อระบบ: เรียกว่า "RiMS" เท่านั้น ห้ามเรียกหรือเขียนว่า "HRiMS" โดยเด็ดขาด
...
PROMPT;
        ...
        try {
            ...
            $answer = "⚠️ **ระบบ AI ยังไม่พร้อมใช้งาน หรือยังไม่ได้ตั้งค่าการเชื่อมต่อ**\n\n"
                    ...
                    . "• **สาเหตุ:** " . $e->getMessage() . "\n\n"
                    . "💡 **คำแนะนำ:** ระบบน้องมีตังค์ (RiMS AI) รองรับการเชื่อมต่อ AI หลากหลายค่าย:\n"
                    ...
                    . "กรุณาเปิดหน้า **ตั้งค่า AI & LLM Connection** เพื่อเลือกผู้ให้บริการ บันทึกค่า หรือทดสอบการเชื่อมต่อค่ะ";
        }
