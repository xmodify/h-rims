            ...
            foreach ($tables as $tbl) {
                ...
                if (!str_starts_with($tbl, 'hosfin_')) {
                    if ($tbl === 'information_schema' || str_contains($tbl, 'schema')) {
                        return [
                            'is_valid' => false,
                            'error' => "ท่านสามารถพิมพ์คำถามเจาะจงที่ต้องการดูข้อมูลได้เลยค่ะ เช่น 'ขอยอดเจ้าหนี้การค้าแยกตามบริษัท', 'ยอดหนี้ค้างจ่าย อภ.', หรือ 'ขอยอดงบทดลองเดือนล่าสุด' โดยน้องมีตังค์จะค้นหาจากฐานข้อมูล HosFin ให้ทันทีค่ะ"
                        ];
                    }
                    return [
                        'is_valid' => false,
