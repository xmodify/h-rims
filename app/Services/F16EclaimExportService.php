...
class F16EclaimExportService
{
    ...
    public static function generate16Files(array $vns, array $options = []): array
    {
        ...
        // -------------------------------------------------------------
        // 9. Query Non-ED Reason / DRUGREMARK from ovst_presc_ned and drugitems_ned_reason
        // -------------------------------------------------------------
        ...
        try {
            ...
            Log::warning("F16 Export drugcat_nhso query error: " . $e->getMessage());
        }
        ...
        // 16. ADP.txt (27 columns ตามโครงสร้างมาตรฐาน e-Claim สปสช.)
    ...
    public static function generate16FilesIp(array $ans, array $options = []): array
    {
        ...
        try {
            $operRows = DB::connection('hosxp')->select("
                SELECT io.an, io.icd9 as oper, COALESCE(io.oper_type, io.ovst_oper_type, '1') as optype, doc.licenseno as dropid,
                       io.opdate as datein, io.optime as timein, io.enddate as dateout, io.endtime as timeout
                FROM iptoprt io
                LEFT JOIN doctor doc ON doc.code = io.doctor
        ...
        // -------------------------------------------------------------
        // 8. Query Non-ED Reason / DRUGREMARK from ipt_presc_ned
        // -------------------------------------------------------------
        ...
        foreach ($groupedDrugs as $it) {
            ...
            $druLines[] = "{$hcode}|{$it->hn}|{$it->an}|{$clinic}|{$cid}|{$dateserv}|{$did}|{$didname}|{$amountStr}|{$drugprice}|{$drugcost}|{$didstd}|{$unit}|{$unitpack}|{$seq}|{$drugremark}|{$pano}|{$totcopay}|{$usestatus}|{$total}|||{$provider}|";
        }
