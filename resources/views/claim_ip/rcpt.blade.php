    ...
    <div class="page-header-box mt-2 mb-3 d-flex justify-content-between align-items-center">
        <div>
            <h4 class="text-primary mb-0 fw-bold">
                ...
                สถิติการชดเชยค่าบริการ IP-ชำระเงินสด
            </h4>
        </div>
            ...
            <div class="d-flex justify-content-between align-items-end mb-3">
                <div class="d-flex align-items-center gap-3">
                    <h6 class="fw-bold text-dark mb-0">
                        <i class="bi bi-people-fill text-primary me-2"></i>รายชื่อผู้มารับบริการ IP-ชำระเงินสด
                    </h6>
            ...
            <ul class="nav nav-tabs-modern" id="pills-tab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="claim-tab" data-bs-toggle="pill" data-bs-target="#claim" type="button" role="tab">
                        <i class="bi bi-send-check me-1"></i> ชำระเงินแล้ว
                    </button>
                </li>
            </ul>
        </div>
                        ...
                        <table id="t_search" class="table table-modern w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center text-primary">ต้องชำระ</th> 
                                    <th class="text-center text-success">ชำระแล้ว</th>
                                    <th class="text-center text-danger">เรียกเก็บ</th>
                            ...
                            <tbody> 
                                ...
                                <tr>
                                    ...
                                    <td class="text-end small text-primary fw-bold">{{ number_format($row->paid_money,2) }}</td>
                                    <td class="text-end small text-success fw-bold">
                                        {{ number_format($row->rcpt_money,2) }}
                                        @if($row->rcpt_money > 0)<div class="small text-muted" style="font-size:0.65rem;">[{{ $row->rcpno }}]</div>@endif
                                    </td>
                                    <td class="text-end small text-danger fw-bold">{{ number_format($row->claim_price,2) }}</td> 
                                    <td class="text-end small">{{ number_format($row->arrear_amount,2) }}</td>               
                                    <td class="text-end small">{{ number_format($row->deposit_amount,2) }}</td>    
                                    <td class="text-end small">{{ number_format($row->debit_amount,2) }}</td>
                            ...
                            <tfoot class="bg-light-soft">
                                <tr>
                                    ...
                                    <th class="text-end small fw-bold text-primary">{{ number_format($sum_paid_money,2) }}</th>
                                    <th class="text-end small fw-bold text-success">{{ number_format($sum_rcpt_money,2) }}</th>
                                    <th class="text-end small fw-bold text-danger">{{ number_format($sum_claim_price,2) }}</th>
                                    <th colspan="5"></th>
                                </tr>
                            </tfoot>
                        </table>
