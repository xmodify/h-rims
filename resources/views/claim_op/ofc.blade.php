...
#claim-tab.active {
    ...
    <div class="page-header-box mt-2 mb-3 d-flex justify-content-between align-items-center">
        <div>
            <h4 class="text-primary mb-0 fw-bold">
                ...
                สถิติการชดเชยค่าบริการ OP-OFC กรมบัญชีกลาง
            </h4>
        </div>
    ...
    <!-- Modal Import EDC (KTB Corporate Online & Manual) -->
    <x-edc_import_modal modal-id="importEdcModal" on-success-callback="loadDashboard" />
            ...
            if (res.success) {
                ...
                var dt_search = $('#t_search').DataTable({
                    ...
                    buttons: [{
                        ...
                        title: 'รายชื่อผู้มารับบริการ OP-OFC รอส่ง Claim'
                    }],
                    language: {
                        search: "ค้นหา:",
                ...
                var dt_claim = $('#t_claim').DataTable({
                    ...
                    buttons: [{
                        ...
                        text: '<i class="bi bi-file-earmark-excel me-1"></i> Excel',
                        className: 'btn btn-success btn-sm shadow-sm',
                        title: 'รายชื่อผู้มารับบริการ OP-OFC ส่ง Claim แล้ว'
                    }],
                    language: {
                        search: "ค้นหา:",
                        lengthMenu: "แสดง _MENU_ รายการ",
            ...
            .done(function(data) {
                ...
                function makeCellHtml(isValid, epDone, warn) {
                    if (!isValid) {
                        ...
                        return `<button class="btn btn-sm btn-outline-warning px-2 py-1 border-2 d-flex align-items-center justify-content-center" style="font-size:0.7rem; height: 26px; min-height: 26px; margin: 0 auto;" onclick="showDetails('${vn}')" title="พบคำเตือน / ยังไม่ปิดสิทธิ | คลิกดูรายละเอียด"><i class="bi bi-eye-fill"></i></button>`;
                    }
                }
                ...
                let endpointBtn = isEndpointDone
                    ? `<span class="text-success fw-bold"><i class="bi bi-check-circle-fill me-1"></i>ปิดสิทธิแล้ว (EDC/E-Claim)</span>`
                    : `<button onclick="pullNhsoData('${visit.vstdate}','${visit.cid}','${vn}')" class="btn btn-warning btn-sm py-1 px-2 fw-bold text-dark" style="font-size:0.75rem;"><i class="bi bi-cloud-download-fill me-1"></i>ดึงข้อมูล (Pull)</button>`;

                let statusHtml = '';
                if (!v.is_valid) {
                    const errorList = v.errors || [];
                    statusHtml = `
                    <div class="col-12">
                      <div class="alert alert-danger py-2 px-3 border-0 shadow-sm d-flex align-items-start small mb-0" style="background-color: #fef2f2; color: #991b1b; border-left: 5px solid #dc2626 !important;">
                        <i class="bi bi-x-circle-fill me-2 mt-1" style="font-size: 1.1rem; color: #dc2626;"></i>
                        <div>
                          <div class="fw-bold mb-1 text-dark">สถานะ: ไม่ผ่านเกณฑ์ส่งออก (มีข้อผิดพลาดที่ต้องแก้ไข)</div>
                          <ul class="mb-0 ps-3 text-danger" style="list-style-type: disc;">${errorList.map(e => `<li>${e}</li>`).join('')}</ul>
                        </div>
                      </div>
                    </div>`;
                } else if (hasWarnings) {
                    const warningsList = v.warnings || [];
                    statusHtml = `
                    <div class="col-12">
                      <div class="alert alert-warning py-2 px-3 border-0 shadow-sm d-flex align-items-start small mb-0" style="background-color: #fffbeb; color: #92400e; border-left: 5px solid #d97706 !important;">
                        ...
                        <div>
                          <div class="fw-bold mb-1 text-dark">สถานะ: พบคำเตือน (ข้อมูลผ่านเกณฑ์แต่ควรตรวจสอบ)</div>
                          <ul class="mb-0 ps-3 text-warning" style="color: #92400e !important; list-style-type: disc;">${warningsList.map(w => `<li>${w}</li>`).join('')}</ul>
                        </div>
                      </div>
                    </div>`;
                } else {
                    ...
                    <div class="col-12">
                      <div class="alert alert-success py-2 px-3 border-0 shadow-sm d-flex align-items-start small mb-0" style="background-color: #f0fdf4; color: #166534; border-left: 5px solid #16a34a !important;">
                        ...
                        <div>
                          <div class="fw-bold mb-1 text-dark">สถานะ: ข้อมูลผ่านเกณฑ์ (ข้อมูลถูกต้องและปิดสิทธิเรียบร้อย)</div>
                        </div>
                      </div>
                    </div>`;
                }
                        ...
                        <table id="modal-services-table" class="table table-sm table-hover align-middle mb-0 small border w-100">
                          <thead class="table-dark">
                            <tr>
                              ...
                              <th class="text-center" width="15%">ประเภทการชำระ</th>
                              <th class="text-center" width="15%">สิทธิการรักษา</th>
                              <th class="text-center" width="10%">ADP CODE</th>
                            </tr>
                          </thead>
                          <tbody>
                            ${(function() {
