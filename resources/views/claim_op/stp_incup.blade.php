...
.badge-herb  { background:#d4edda; color:#155724; }
    ...
    <div class="page-header-box mt-2 mb-3 d-flex justify-content-between align-items-center">
        <div>
            <h4 class="text-primary mb-0 fw-bold">
                ...
                สถิติการชดเชยค่าบริการ STP-OP ใน CUP
            </h4>
        </div>
    ...
    <x-import_hub_modal 
        :has-fdh="true"
        claim-title="สิทธิบุคคลผู้มีปัญหาสถานะฯ (STP) ใน CUP" 
    />
...
@push('scripts')
  ...
  <script>
    let myChart = null;
    const VISIT_DETAILS_URL = "{{ url('claim_op/stp_incup/visit_details') }}";
    ...
    function exportSingleVnFdh() {
        ...
        openF16FdhExportModal({
            ...
            ans: [currentModalVn],
            claimCode: 'STP_INCUP',
            claimTitle: 'สิทธิบุคคลผู้มีปัญหาสถานะฯ (STP) ใน CUP',
            isIp: false
        });
    }
    ...
    function showDetails(vn) {
        ...
        .done(function(data) {
            ...
            function renderModalTables(onlyPpfs) {
                ...
                if (onlyPpfs) {
                    drugsList = drugsList.filter(d => d.ppfs === 'Y');
                }

                let servicesList = window.currentModalItems.filter(d => !d.icode.startsWith('1'));
                if (onlyPpfs) {
                    servicesList = servicesList.filter(d => d.ppfs === 'Y');
                }
              ...
              window.patientItems = res.patient_items || [];
                ...
                var dt_visits = $('#t_visits').DataTable({
                    ...
                    buttons: [{
                        ...
                        title: 'รายชื่อผู้มารับบริการ STP-OP ใน CUP'
                    }],
                    language: {
                        search: "ค้นหา:",
                        lengthMenu: "แสดง _MENU_ รายการ",
