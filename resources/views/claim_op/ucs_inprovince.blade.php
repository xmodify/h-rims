...
/* Custom pastel background for main tabs in ucs_inprovince */
#search-tab {
...
#claim-tab.active {
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
          ...
          <div class="modal-footer bg-white border-top py-2 px-4 d-flex justify-content-between">
            ...
            <div class="d-flex gap-2">
              <button type="button" class="btn text-white fw-bold px-4 shadow-sm" style="background: linear-gradient(135deg, #0e939a 0%, #15b7bd 100%); border: none;" onclick="exportSingleVnFdh()">
    ...
    <x-import_hub_modal 
        ...
        :has-edc="false" 
        :has-fdh="true"
        claim-title="สิทธิ UC-OP นอก CUP ในจังหวัด" 
    />
...
@push('scripts')
  ...
  <script>
    ...
    function loadDashboard(dataParams) {
        ...
        // Save active tab category clicked
        const currentActiveTab = $('.nav-tabs-modern .nav-link.active').attr('data-bs-target') || '#search';

        $.ajax({
            url: "{{ url('claim_op/ucs_inprovince') }}",
            type: "POST",
            ...
            success: function(res) {
                ...
                var dt_search = $('#t_search').DataTable({
                    ...
                    buttons: [{
                        ...
                        title: 'รายชื่อผู้มารับบริการ UC-OP ในจังหวัด รอส่ง Claim'
                    }],
                    language: {
                        search: "ค้นหา:",
                ...
                var dt_claim = $('#t_claim').DataTable({
                    ...
                    buttons: [{
                        ...
                        text: '<i class="bi bi-file-earmark-excel me-1"></i> Excel',
                        className: 'btn btn-success btn-sm',
                        title: 'รายชื่อผู้มารับบริการ UC-OP ในจังหวัด ส่ง Claim แล้ว'
                    }],
                    language: {
                        search: "ค้นหา:",
                        lengthMenu: "แสดง _MENU_ รายการ",
    ...
    function showValidation(vn) {
        ...
        $.get(VISIT_DETAILS_URL, { vn: vn })
            ...
            .fail(function() {
                body.innerHTML = '<div class="alert alert-danger">ไม่สามารถดึงข้อมูลการตรวจสอบได้</div>';
            });
    }
    ...
    function exportSingleVnFdh() {
        ...
        openF16FdhExportModal({
            vns: [currentModalVn],
            ans: [currentModalVn],
            claimCode: 'UCS_INPROV',
            claimTitle: 'สิทธิ UC-OP นอก CUP ในจังหวัด',
            isIp: false
        });
    }
    ...
    function showDetails(vn) {
        ...
        body.innerHTML = '<div class="text-center text-muted py-4"><i class="bi bi-arrow-repeat spin me-2"></i>กำลังโหลดรายละเอียดการรับบริการ...</div>';
        ...
        $.get(VISIT_DETAILS_URL, { vn: vn })
            .done(function(data) {
                ...
                if (!v.is_valid) {
                    ...
                    statusBadge = `
                      <div class="alert alert-warning d-flex align-items-start gap-2 py-2 px-3 mb-3" style="font-size:0.8rem;">
                        <i class="bi bi-exclamation-triangle-fill fs-5 mt-1"></i>
                        <div>
                          <div class="fw-bold">สถานะ: ข้อมูลพร้อมส่งออก (ยังไม่ปิดสิทธิ สปสช.)</div>
                          <div class="small opacity-90">กรุณากดดึงสิทธิ หรือทำการส่งข้อมูลปิดสิทธิเพื่อความถูกต้อง</div>
                        </div>
                      </div>`;
                } else {
                    statusBadge = `
                      <div class="alert alert-success d-flex align-items-start gap-2 py-2 px-3 mb-3" style="font-size:0.8rem;">
                        <i class="bi bi-check-circle-fill fs-5 mt-1"></i>
                        <div>
                          <div class="fw-bold">สถานะ: ข้อมูลพร้อมส่งออก (ผ่านเกณฑ์และปิดสิทธิเรียบร้อย)</div>
                          <div class="small opacity-90">ข้อมูลถูกต้องครบถ้วนและทำการปิดสิทธิเรียบร้อยแล้ว</div>
                        </div>
                      </div>`;
                }

                // Check claim code and action buttons
                let endpointBtn = '';
                if (v.endpoint_valid) {
                    ...
                    <div class="col-md-4">
                      <div class="card border-0 bg-light-soft h-100">
                        <div class="card-body py-2 px-3">
                          <div class="fw-bold text-primary mb-2 small"><i class="bi bi-person-badge me-1"></i>ข้อมูลผู้ป่วย</div>
                          <table class="table table-sm table-borderless mb-0 small compact-info-table">
                            <tr><th class="text-muted" style="width:30%">HN</th><td class="fw-bold text-dark">${visit.hn}</td></tr>
    ...
    // ── FDH Individual Status Checking ──────────────────────────────────────
    ...
    // ── App Init & Form Bindings ────────────────────────────────────────────
    $(document).ready(function () {
        // Initial dashboard full load
        loadDashboard({
        ...
        $(document).on('submit', '#form_budget_year', function(e) {
            e.preventDefault();
            const chosenYear = $(this).find('select[name="budget_year"]').val();
            $('#display-budget-year').text(chosenYear);
            loadDashboard({
                budget_year: chosenYear
            });
        });
    ...
    function exportSelectedF16FDH(claimCode) {
        ...
        openF16FdhExportModal({
            vns: selectedVns,
            claimCode: claimCode || 'UCS_INPROV',
            claimTitle: 'สิทธิ UC-OP ในจังหวัด',
            isIp: false
        });
    }
