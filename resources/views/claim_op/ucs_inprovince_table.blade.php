                ...
                <div class="filter-group">
                    <form id="form_indiv" method="POST" enctype="multipart/form-data" class="m-0 d-flex align-items-center">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <button type="button" class="btn text-white fw-bold px-3 shadow-sm" style="background: linear-gradient(135deg, #0e939a 0%, #15b7bd 100%); border: none;" onclick="exportSelectedF16FDH('UCS_INPROV')">
                    ...
                    <div class="table-responsive">            
                        <table id="t_search" class="table table-modern w-100">
                            ...
                            <tbody> 
                                ...
                                @php
                                    $search_items[] = ['hn' => $row->hn, 'seq' => $row->seq];
                                @endphp
                                <tr>
                    ...
                    <div class="table-responsive">            
                        <table id="t_claim" class="table table-modern w-100">
                            ...
                            <tbody> 
                                ...
                                @php
                                    $claim_items[] = ['hn' => $row->hn, 'seq' => $row->seq];
                                @endphp
                                <tr>
                                    <td class="text-center" style="vertical-align: middle;">
                                        <input type="checkbox" class="form-check-input f16-select-item" value="{{ $row->vn ?? $row->seq }}" data-vn="{{ $row->vn ?? $row->seq }}">
                            ...
                            <tfoot class="bg-light-soft">
                                <tr>
                                    <th colspan="{{ $is_f16_licensed ? 10 : 9 }}" class="text-end text-muted small px-3">รวมงบประมาณที่ส่งเบิก:</th>
                                    <th class="text-end small">{{ number_format($sum_income,2) }}</th>
                                    <th class="text-end small">{{ number_format($sum_rcpt_money,2) }}</th>
...
<script>
    // Bind current search page patient items to window object
    window.patientItems = {!! json_encode(array_merge($search_items, $claim_items)) !!};
</script>
