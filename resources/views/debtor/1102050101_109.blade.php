    ...
    <div class="page-header-box mt-2 mb-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center flex-wrap">
        ...
        <div class="d-flex align-items-center gap-4">
            ...
            <div class="filter-group">
                <form method="POST" action="{{ url('debtor/1102050101_109') }}" enctype="multipart/form-data" class="m-0 d-flex flex-wrap align-items-center gap-2">
                    @csrf
        ...
        <div class="card-body px-4 pb-4 pt-0">
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade show active" id="debtor-pane" role="tabpanel"> 
                    <form id="form-delete" action="{{ url('debtor/1102050101_109_delete') }}" method="POST">
                        @csrf   
                        @method('DELETE')
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <div>
                        <a class="btn btn-outline-success btn-sm" href="{{ url('debtor/1102050101_109_indiv_excel')}}" target="_blank">
                             <i class="bi bi-file-earmark-excel me-1"></i> ส่งออกรายตัว
                        </a>                
                        <a class="btn btn-outline-primary btn-sm" href="{{ url('debtor/1102050101_109_daily_pdf')}}" target="_blank">
                ...
                <div class="table-responsive"><table id="debtor" class="table table-bordered table-striped my-3" width="100%">
                    ...
                    <tr class="table-primary align-middle">
                        <th colspan="8" class="text-center">1102050101.109-ลูกหนี้-ระบบปฏิบัติการฉุกเฉิน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th>
                    ...
                    @php 
                        $sum_inc = 0; $sum_rcp = 0;
                        $sum_debtor = 0; $sum_receive = 0; $sum_adj_inc = 0; $sum_adj_dec = 0; $sum_balance = 0; 
                    @endphp
                    ...
                    <tr class="align-middle">
                        ...
                        <td align="right" style="color:{{$row->receive >= 0 ? 'green' : 'red'}}">
                        ...
                        <td align="center">         
                            <button type="button" class="btn btn-warning btn-sm px-2 shadow-sm text-dark" data-bs-toggle="modal" data-bs-target="#receive-{{ str_replace('/', '-', $row->vn) }}" title="ชดเชย/ปรับปรุง"> 
                    ...
                    <tfoot>
                        <tr class="table-success text-end fw-bold" style="font-size: 14px;">
                            ...
                            <td style="color:green">{{ number_format($sum_receive,2) }}</td>
        ...
        <div id="receive-{{ str_replace('/', '-', $row->vn) }}" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    ...
                    <form action="{{ url('debtor/1102050101_109/update', $row->vn) }}" method="POST">
                        @csrf @method('PUT')
                                ...
                                <div class="col-md-6 border-end">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6 mb-2">
                                            ...
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="charge_date_{{ $row->vn }}" value="{{ !empty($row->charge_date) ? DateThai($row->charge_date) : '' }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6 mb-2">
                                            ...
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="receive_date_{{ $row->vn }}" value="{{ !empty($row->receive_date) ? DateThai($row->receive_date) : '' }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="row g-2">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="adj_date_{{ $row->vn }}" value="{{ !empty($row->adj_date) ? DateThai($row->adj_date) : DateThai(date('Y-m-d')) }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-3">
...
<!-- ลบลูกหนี้ -->
    <script>
        function confirmDelete() {
            ...
            Swal.fire({title:'ยืนยัน?',text:"ต้องการลบรายการที่เลือกใช่หรือไม่?",icon:'warning',showCancelButton:true,confirmButtonColor:'#d33',confirmButtonText:'ใช่, ลบเลย!',cancelButtonText:'ยกเลิก'}).then((r)=>{ if(r.isConfirmed){ document.getElementById('form-delete').submit(); }});
        }
        function confirmSubmit() {
            const sel = [...document.querySelectorAll('input[name="checkbox[]"]:checked')].map(e=>e.value);
            if(!sel.length) { Swal.fire('แจ้งเตือน','กรุณาเลือกรายการ','warning'); return; }
            Swal.fire({title:'ยืนยัน?',text:"ต้องการยืนยันรายการที่เลือกใช่หรือไม่?",icon:'question',showCancelButton:true,confirmButtonColor:'#28a745',confirmButtonText:'ยืนยัน',cancelButtonText:'ยกเลิก'}).then((r)=>{ if(r.isConfirmed){ document.getElementById('form-confirm').submit(); }});
        }
    ...
    <script>
        function bulkAdjust() {
            ...
            Swal.fire({
                ...
                if (r.isConfirmed) {
                    showLoading(); let f=document.createElement('form'); f.method='POST'; f.action="{{ url('debtor/1102050101_109_bulk_adj') }}";
