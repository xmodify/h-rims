...
@section('content')
    ...
    <div class="page-header-box mt-2 mb-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center flex-wrap">
        ...
        <div class="d-flex align-items-center gap-4">
            ...
            <div class="filter-group">
                <form method="POST" action="{{ url('debtor/1102050102_110') }}" enctype="multipart/form-data" class="m-0 d-flex flex-wrap align-items-center gap-2">
                    @csrf
        ...
        <div class="card-body px-4 pb-4 pt-0">
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade show active" id="debtor-pane" role="tabpanel"> 
                    <form id="form-delete" action="{{ url('debtor/1102050102_110_delete') }}" method="POST" enctype="multipart/form-data">
                        @csrf   
                        @method('DELETE')
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2 mt-3">
                            ...
                            <div>
                                <a class="btn btn-outline-success btn-sm" href="{{ url('debtor/1102050102_110_indiv_excel')}}" target="_blank">
                                    <i class="bi bi-file-earmark-excel me-1"></i> ส่งออกรายตัว
                                </a>                
                                <a class="btn btn-outline-primary btn-sm" href="{{ url('debtor/1102050102_110_daily_pdf')}}" target="_blank">
                        ...
                        <div class="table-responsive"><table id="debtor" class="table table-bordered table-striped my-3" width="100%">
                            ...
                            <tr class="table-success">
                                <th class="text-left text-primary" colspan = "12">1102050102.110-ลูกหนี้ค่ารักษา เบิกจ่ายตรงหน่วยงานอื่น OP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
                            ...
                            <tbody>
                            <?php 
                                $count = 1; $sum_income = 0; $sum_rcpt_money = 0; $sum_ofc = 0; $sum_kidney = 0; $sum_ppfs = 0; $sum_other = 0;
                                $sum_debtor = 0; $sum_receive = 0; $sum_receive_ppfs = 0;
                            ...
                            <tr>
                                ...
                                <td align="right" style="color:green">{{ number_format($row->receive_ppfs,2) }}</td>
                            ...
                            <?php 
                                $count++; $sum_income += $row->income; $sum_rcpt_money += $row->rcpt_money; $sum_ofc += $row->ofc;
                                $sum_kidney += $row->kidney; $sum_ppfs += $row->ppfs; $sum_other += $row->other; $sum_debtor += $row->debtor;
                                $sum_receive += $row->receive; $sum_receive_ppfs += $row->receive_ppfs;
                            ...
                            <tfoot>
                                <tr class="table-success text-end" style="font-weight:bold; font-size: 14px;">
                                    ...
                                    <td class="text-end">{{ number_format($sum_ofc,2) }}</td>
                ...
                <div class="tab-pane fade" id="confirm-pane" role="tabpanel">
                    <form id="form-confirm" action="{{ url('debtor/1102050102_110_confirm') }}" method="POST" enctype="multipart/form-data">
                        @csrf                
                        ...
                        <div class="table-responsive"><table id="debtor_search" class="table table-bordered table-striped my-3" width="100%">
                            ...
                            <tr class="table-secondary">
                                <th class="text-left text-primary" colspan = "17">1102050102.110-ลูกหนี้ค่ารักษา เบิกจ่ายตรงหน่วยงานอื่น OP รอยืนยัน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                            </tr>
                            <tr class="table-secondary">
                            ...
                            <tr>
                                ...
                                <td align="center">         
                                    <button type="button" class="btn btn-warning btn-sm px-2 shadow-sm text-dark" data-bs-toggle="modal" data-bs-target="#receive-confirm-{{ str_replace('/', '-', $row->vn) }}"> 
                                        <i class="bi bi-pencil-square"></i>
                                    </button>                            
                                </td>
                            <?php $count++; ?>
        ...
        <div id="receive-{{ str_replace('/', '-', $row->vn) }}" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    ...
                    <form action="{{ url('debtor/1102050102_110/update', $row->vn) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-6 border-end">
                                    ...
                                    <div class="row g-2">
                                        ...
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">เลขที่หนังสือเรียกเก็บ</label>
                                        ...
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">จำนวนเงินที่เรียกเก็บ</label>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="row g-2">
                                        ...
                                        <div class="col-md-6 mb-0">
                                            ...
                                            <div class="input-group input-group-sm">
                                                <input type="number" step="0.01" class="form-control rounded-pill-start px-3" name="receive" value="{{ $row->receive_manual }}">
                                                <span class="input-group-text rounded-pill-end small bg-light">บาท</span>
                                            </div>
                                        </div>
                                        <div class="col-md-6 mb-0">
    ...
    @foreach($debtor_search as $row)
        @php
            $balance = ($row->receive + ($row->adj_inc ?? 0) - ($row->adj_dec ?? 0)) - $row->debtor;
        @endphp
        <div id="receive-confirm-{{ str_replace('/', '-', $row->vn) }}" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-primary text-white border-0 py-3">
                        <h5 class="modal-title d-flex align-items-center"><i class="bi bi-cash-stack me-2"></i> รายการการชดเชยเงิน/ลูกหนี้ (VN: {{ $row->vn }})</h5>
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-6 border-end">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">วันที่เรียกเก็บ</label>
                                            <input type="hidden" name="charge_date" id="charge_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ $row->charge_date }}">
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="charge_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ !empty($row->charge_date) ? DateThai($row->charge_date) : '' }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">เลขที่หนังสือเรียกเก็บ</label>
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3" name="charge_no" value="{{ $row->charge_no }}" placeholder="ระบุเลขที่หนังสือ">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">วันที่ชดเชย</label>
                                            <input type="hidden" name="receive_date" id="receive_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ $row->receive_date }}">
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="receive_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ !empty($row->receive_date) ? DateThai($row->receive_date) : '' }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                            <label class="form-label mb-1 small fw-bold">เลขที่หนังสือชดเชย</label>
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3" name="receive_no" value="{{ $row->receive_no }}" placeholder="ระบุเลขที่โอน">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="row g-2">
                                        ...
                                        <div class="col-md-3">
                                            <label class="form-label mb-1 small fw-bold text-muted">วันที่ปรับปรุง</label>
                                            <input type="hidden" name="adj_date" id="adj_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ $row->adj_date ?? date('Y-m-d') }}">
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3 datepicker_th" data-hidden-id="adj_date_confirm_{{ str_replace(['/','.'], '-', $row->vn) }}" value="{{ !empty($row->adj_date) ? DateThai($row->adj_date) : DateThai(date('Y-m-d')) }}" placeholder="วว/ดด/ปปปป" readonly>
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label mb-1 small fw-bold text-muted">หมายเหตุ</label>
                                            <input type="text" class="form-control form-control-sm rounded-pill px-3" name="adj_note" value="{{ $row->adj_note }}" placeholder="ระบุเหตุผล">
    ...
    <script>
        ...
        function bulkAdjust() {
            ...
            if (selected.length === 0) {
                Swal.fire('แจ้งเตือน', 'กรุณาเลือกรายการที่ต้องการปรับปรุงยอด', 'warning');
                return;
            }
            Swal.fire({
        ...
        function confirmUnlock(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = "{{ url('debtor/1102050102_110/unlock') }}/" + id;
        ...
        function confirmLock(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var form = document.createElement('form');
                    form.method = 'POST';
                    form.action = "{{ url('debtor/1102050102_110/lock') }}/" + id;
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('changeDate', '.datepicker_th', function(e) {
                ...
                var hiddenInput = hiddenId ? $('#' + hiddenId) : $(this).prev('input[type="hidden"]');
