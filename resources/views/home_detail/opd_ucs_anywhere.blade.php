    ...
    <div style="overflow-x:auto;">            
      <table id="t_search" class="table table-striped table-bordered" width = "100%">
        ...
        <tbody> 
          ...
          <tr>
            ...
            <td align="center" @if($row->receive_total-$row->debtor > 0) style="color:green" 
                @elseif($row->receive_total-$row->debtor < 0) style="color:red" @endif>
                {{ number_format($row->receive_total-$row->debtor,2) }}</td>
      ...
      <div>
        <h5 class="text-primary text-center">
          ...
          เบิกได้: <strong>{{ number_format($sum_debtor,2)}}</strong> บาท |
          ...
          ผลต่าง: <strong  @if($sum_receive_total-$sum_debtor > 0) style="color:green" 
                    @elseif($sum_receive_total-$sum_debtor < 0) style="color:red" @endif>
                    {{ number_format($sum_receive_total-$sum_debtor,2)}}</strong> บาท
        </h5>
      </div>   
    </div>
  </div> 
        ...
        buttons: [
            {
              ...
              title: 'รายชื่อผู้มารับบริการ UC-OP Anywhere วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}'
            }
        ],
        language: {
            search: "ค้นหา:",
