...
<div class="container-fluid px-lg-4">
    ...
    <div class="page-header-box mt-3 mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3 p-3 bg-white shadow-sm" style="border-radius: 14px;">
        <div>
            <h5 class="text-dark mb-0 fw-bold">
                ...
                รายละเอียดการตรวจสอบเบื้องต้น (REP) สิทธิ์เจ้าหน้าที่ ขสมก. (BMT) BMT [OP-IP]
            </h5>
    ...
    <!-- OPD Data Table Card -->
    <div class="card dash-card border-0 shadow-sm mb-4" style="border-radius: 14px; border-top: 4px solid #fd7e14 !important;">
        <div class="card-header bg-transparent border-0 pb-0 pt-4 px-4">
            <h6 class="fw-bold text-dark mb-0"><i class="bi bi-person-badge me-2 text-warning"></i> ผู้ป่วยนอก OP</h6>
        </div>
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="rep_bmt_list" class="table table-modern w-100">
                    <thead>
                        <tr>
    ...
    <div class="card dash-card border-0 shadow-sm mb-4" style="border-radius: 14px; border-top: 4px solid #ef4444 !important;">
        ...
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="rep_bmt_list_ip" class="table table-modern w-100">
                    <thead>
                        <tr>
                            <th class="text-center">ประเภท</th>
                            <th class="text-center">เลขที่ REP</th> 
...
<script>
    $(document).ready(function () {
      ...
      // OPD DataTable
      $('#rep_bmt_list').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ url('import/rep_bmt_detail') }}",
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'net_compensate_nhso', name: 'net_compensate_nhso', className: 'text-end text-warning fw-bold' },
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    ...
                    window.location.href = "{{ url('import/rep_bmt_detail') }}?export=excel&type=opd&start_date=" + start + "&end_date=" + end;
                }
            }
        ],
        language: {
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    var start = $('#start_date').val();
                    var end = $('#end_date').val();
                    window.location.href = "{{ url('import/rep_bmt_detail') }}?export=excel&type=ipd&start_date=" + start + "&end_date=" + end;
                }
            }
        ],
        language: {
