        ...
        <div class="col-md-8">
            <div class="card dash-card border-0 shadow-sm" style="border-top: 4px solid #0d6efd !important; border-radius: 14px;">
                <div class="card-body p-4">
                    <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('import/rep_lgo_save') }}" method="POST" enctype="multipart/form-data" class="m-0">
                        @csrf
                        <div class="text-center mb-3">
                            <h6 class="fw-bold text-dark"><i class="bi bi-file-earmark-excel me-2 text-primary"></i> นำเข้าไฟล์ REP ท้องถิ่น (อปท.) (Excel Only)</h6>
    ...
    <div class="page-header-box mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3 p-3 bg-white shadow-sm" style="border-radius: 14px;">
        <div>
            <h5 class="text-dark mb-0 fw-bold text-truncate" style="max-width: 500px;">
                ...
                ข้อมูลการตรวจสอบเบื้องต้น (REP) สิทธิ์ท้องถิ่น (อปท.) LGO [OP-IP]
            </h5>
            <div class="text-muted small mt-1">ปีงบประมาณประจำปัจจุบัน: {{ $budget_year }}</div>
            <div class="mt-2 d-flex gap-2 flex-wrap">
                <a href="{{ url('/import/rep_lgo_detail_opd') }}" class="btn btn-primary btn-sm rounded-pill px-3 shadow-sm">
                    <i class="bi bi-person-badge me-1"></i> รายละเอียด OPD
                </a>
                <a href="{{ url('/import/rep_lgo_detail_ipd') }}" class="btn btn-danger btn-sm rounded-pill px-3 shadow-sm">
    ...
    <div class="card dash-card border-0 shadow-sm mb-4" style="border-radius: 14px;">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="rep_lgo_table" class="table table-modern w-100">
                    <thead>
                        <tr>
            ...
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="text-muted small" id="db_subtitle">ยอดชดเชยสุทธิรายเดือน REP สิทธิ์ท้องถิ่น (อปท.) LGO</div>
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="modal-title fw-bold mb-0" id="eclaimRepBotModalLabel">
                            ดึงข้อมูลการตรวจสอบเบื้องต้น (REP LGO (สิทธิ์ อปท.)) จาก e-Claim อัตโนมัติ
                        </h5>
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-white py-2.5 px-3 d-flex justify-content-between align-items-center">
                        <div class="fw-bold small text-dark d-flex align-items-center gap-2">
                            <i class="bi bi-list-task text-success"></i> รายการ REP ที่พบใน e-Claim (REP LGO (สิทธิ์ อปท.))
                        </div>
...
@push('scripts')
  <script>
        ...
        $(document).ready(function () {
            ...
            $('#rep_lgo_table').DataTable({
                ...
                buttons: [
                {
                    ...
                    title: 'ข้อมูล REP สิทธิ์ท้องถิ่น (อปท.) LGO [OP-IP]'
                }
                ],
                language: {
                    search: "ค้นหา:",
            ...
            function loadChartData() {
                ...
                $('#db_subtitle').text(`ยอดชดเชยสุทธิรายเดือน REP สิทธิ์ท้องถิ่น (อปท.) LGO ปีงบประมาณ: ${budgetYearText}`);

                $('#chart_container').addClass('d-none');
                $('#loading_spinner').removeClass('d-none');

                $.ajax({
                    url: "{{ route('import.rep_lgo.chart-data') }}",
                    method: "GET",
                    data: {
            ...
            $(document).on('click', '.fail-link', function () {
                ...
                // AJAX Request
                $.ajax({
                    url: "{{ route('import.rep_lgo.fail-details') }}",
                    method: "GET",
                    data: {
            ...
            function loadCCodeChartData() {
                ...
                $.ajax({
                    url: "{{ route('import.rep_lgo.c-code-chart-data') }}",
                    method: "GET",
                    data: {
                        budget_year: budgetYear,
                        type: serviceType
        ...
        // ==========================================
        // e-Claim REP Automation (lgo)
        // ==========================================
        var repMaininscl = 'lgo';
