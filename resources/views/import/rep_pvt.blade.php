...
<style>
    ...
    .btn-pink:hover {
        background-color: #b92a71 !important;
        border-color: #b92a71 !important;
    }
    .text-pink {
...
<div class="container-fluid px-lg-4">
    ...
    <div class="row justify-content-center mt-3 mb-4">
        <div class="col-md-8">
            <div class="card dash-card border-0 shadow-sm" style="border-top: 4px solid #d63384 !important; border-radius: 14px;">
                <div class="card-body p-4">
                    <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('import/rep_pvt_save') }}" method="POST" enctype="multipart/form-data" class="m-0">
                        @csrf
                        <div class="text-center mb-3">
                            <h6 class="fw-bold text-dark"><i class="bi bi-file-earmark-excel me-2 text-pink"></i> นำเข้าไฟล์ REP สิทธิ์ครูโรงเรียนเอกชน (PVT) (Excel Only)</h6>
                        ...
                        <div class="input-group mb-0">
                            ...
                            <button class="btn btn-pink px-3.5" type="submit" style="border-radius: 0;">
    ...
    <div class="page-header-box mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3 p-3 bg-white shadow-sm" style="border-radius: 14px;">
        <div>
            <h5 class="text-dark mb-0 fw-bold text-truncate" style="max-width: 500px;">
                <i class="bi bi-cloud-arrow-down-fill text-pink me-2"></i>
                ข้อมูลการตรวจสอบเบื้องต้น (REP) สิทธิ์ครูโรงเรียนเอกชน (PVT) PVT [OP-IP]
            </h5>
            <div class="text-muted small mt-1">ปีงบประมาณประจำปัจจุบัน: {{ $budget_year }}</div>
            <div class="mt-2 d-flex gap-2 flex-wrap">
                <a href="{{ url('/import/rep_pvt_detail_opd') }}" class="btn btn-pink btn-sm rounded-pill px-3 shadow-sm">
                    <i class="bi bi-person-badge me-1"></i> รายละเอียด OPD
                </a>
                <a href="{{ url('/import/rep_pvt_detail_ipd') }}" class="btn btn-danger btn-sm rounded-pill px-3 shadow-sm">
        ...
        <form method="POST" enctype="multipart/form-data" class="m-0">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <button type="submit" class="btn btn-pink btn-sm rounded-pill px-3">ค้นหา</button>
            </div>
        </form>
    </div>
    ...
    <div class="card dash-card border-0 shadow-sm mb-4" style="border-radius: 14px;">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="rep_pvt_table" class="table table-modern w-100">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end text-pink fw-bold">{{ number_format($row->receive_total,2) }}</td>
                            @if(Auth::user()->status == 'admin')
                    ...
                    <tfoot>
                        <tr class="fw-bold bg-light" style="border-top: 2px solid #dee2e6 !important;">
                            ...
                            <td class="text-end text-pink">{{ number_format($total_receive,2) }}</td>
                            @if(Auth::user()->status == 'admin')
                                <td></td>
                            @endif
                        </tr>
        ...
        <div class="modal-content shadow-lg border-0" style="border-radius: 20px;">
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center">
                    <div class="icon-box bg-pink text-white mb-0 me-3" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center; border-radius: 12px;">
                        <i class="bi bi-bar-chart-fill fs-5"></i>
                    </div>
                    <div>
                        <h5 class="modal-title fw-bold text-dark" id="db_title">Dashboard</h5>
                        <div class="text-muted small" id="db_subtitle">ยอดชดเชยสุทธิรายเดือน REP สิทธิ์ครูโรงเรียนเอกชน (PVT) PVT</div>
                    </div>
                </div>
            ...
            <div class="modal-body p-4">
                <div class="row mb-4 align-items-center">
                    <div class="col-md-4">
                        <span class="badge bg-pink-subtle text-pink px-3 py-2 rounded-pill">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        Swal.fire({
            ...
            customClass: {
                confirmButton: 'btn btn-pink btn-sm px-4'
            },
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="modal-title fw-bold mb-0" id="eclaimRepBotModalLabel">
                            ดึงข้อมูลการตรวจสอบเบื้องต้น (REP PVT (สิทธิ์ครูโรงเรียนเอกชน)) จาก e-Claim อัตโนมัติ
                        </h5>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-body p-3">
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <button type="button" class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm" id="btnEclaimRepLoginPopup">
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-white py-2.5 px-3 d-flex justify-content-between align-items-center">
                        <div class="fw-bold small text-dark d-flex align-items-center gap-2">
                            <i class="bi bi-list-task text-success"></i> รายการ REP ที่พบใน e-Claim (REP PVT (สิทธิ์ครูโรงเรียนเอกชน))
                        </div>
...
@push('scripts')
  <script>
        ...
        $(document).ready(function () {
            ...
            $('#rep_pvt_table').DataTable({
                ...
                buttons: [
                {
                    ...
                    title: 'ข้อมูล REP สิทธิ์ครูโรงเรียนเอกชน (PVT) PVT [OP-IP]'
                }
                ],
                language: {
                    search: "ค้นหา:",
            ...
            function loadChartData() {
                ...
                $('#db_subtitle').text(`ยอดชดเชยสุทธิรายเดือน REP สิทธิ์ครูโรงเรียนเอกชน (PVT) PVT ปีงบประมาณ: ${budgetYearText}`);

                $('#chart_container').addClass('d-none');
                $('#loading_spinner').removeClass('d-none');

                $.ajax({
                    url: "{{ route('import.rep_pvt.chart-data') }}",
                    method: "GET",
                    data: {
            ...
            $(document).on('click', '.fail-link', function () {
                ...
                // AJAX Request
                $.ajax({
                    url: "{{ route('import.rep_pvt.fail-details') }}",
                    method: "GET",
                    data: {
            ...
            function loadCCodeChartData() {
                ...
                $.ajax({
                    url: "{{ route('import.rep_pvt.c-code-chart-data') }}",
                    method: "GET",
                    data: {
                        budget_year: budgetYear,
                        type: serviceType
            ...
            function renderYearlyCCodeChart(labels, counts) {
                const options = {
                    ...
                    colors: ['#dc3545', '#fd7e14', '#ffc107', '#20c997', '#d63384', '#6610f2', '#d63384', '#00f5ff', '#32cd32', '#ba55d3'],
                    dataLabels: {
                        enabled: true,
                        formatter: function (val) {
        ...
        // ==========================================
        // e-Claim REP Automation (pvt)
        // ==========================================
        var repMaininscl = 'pvt';

        function checkEclaimRepStatus() {
            $('#eclaimRepAuthStatusIcon').removeClass('bg-success-subtle text-success bg-warning-subtle text-warning').addClass('bg-secondary-subtle text-secondary')
        ...
        $('#eclaimRepBotModal').on('hidden.bs.modal', function () {
            if (window.eclaimRetryTimer_checkEclaimRepStatus) {
                ...
                window.eclaimRetryTimer_checkEclaimRepStatus = null;
                        ...
                        $('#eclaimRepAuthStatusIcon').removeClass('bg-success-subtle text-success bg-secondary-subtle text-secondary').addClass('bg-warning-subtle text-warning')
