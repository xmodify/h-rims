    ...
    <div class="row justify-content-center mt-3 mb-4">
        <div class="col-md-8">
            <div class="card dash-card accent-9">
                <div class="card-body">
                    <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('import/stm_bmt_save') }}" method="POST" enctype="multipart/form-data" class="m-0">
                        @csrf
                        <div class="text-center mb-3">
                            <h6 class="fw-bold text-dark"><i class="bi bi-file-earmark-excel me-2 text-success"></i> นำเข้าไฟล์ STM ขสมก. BMT (Excel Only)</h6>
    ...
    <div class="page-header-box">
        <div>
            <h5 class="text-dark mb-0 fw-bold">
                ...
                ข้อมูล Statement เบิกจ่ายตรง ขสมก. BMT
            </h5>
            <div class="text-muted small mt-1">ปีงบประมาณประจำปัจจุบัน: {{ $budget_year }}</div>
            <div class="mt-2 d-flex gap-2">
                <a href="{{ url('/import/stm_bmt_detail_opd') }}" class="btn btn-primary btn-sm rounded-pill px-3">
    ...
    <div class="card dash-card border-top-0">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="stm_bmt" class="table table-modern w-100">
                    <thead>
                        <tr>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div class="modal-header text-white p-3 px-4 border-0" style="background: linear-gradient(135deg, #0f172a 0%, #0369a1 100%) !important;">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="modal-title fw-bold mb-0" id="eclaimStmBmtBotModalLabel">
                            ดึงข้อมูล Statement ขสมก. (BMT) อัตโนมัติจาก e-Claim
                        </h5>
                        <div class="small opacity-75 mt-0.5 d-flex align-items-center gap-2">
                            <span>เชื่อมต่อระบบ e-Claim สปสช. (ระบบรายงานสิทธิเจ้าหน้าที่ ขสมก.) เพื่อค้นหาและนำเข้า Statement อัตโนมัติ</span>
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('btnSaveReceipt')
                .addEventListener('click', function () {
                    ...
                    fetch("{{ url('import/stm_bmt_updateReceipt') }}", {
                        method: "POST",
                        headers: {
        ...
        $(document).ready(function () {
            ...
            var stmTable = $('#stm_bmt').DataTable({
                ordering: false,
                dom: '<"row mb-3"' +
                        '<"col-md-6"l>' +
                ...
                buttons: [
                {
                    ...
                    title: 'ข้อมูล Statement เบิกจ่ายตรง ขสมก. BMT'
                }
                ],
                language: {
                    search: "ค้นหา:",
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex, rowData, counter) {
                    if (settings.sTableId !== 'stm_bmt') return true;
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res && res.connected) {
                        ...
                        $('#btnEclaimStmBmtLoginPopup').html('<i class="bi bi-arrow-repeat me-1"></i> เข้าสู่ระบบใหม่ / เปลี่ยนบัญชี ThaiD');
                        ...
                        $('#btnEclaimStmBmtLoginPopup').html('<i class="bi bi-qr-code-scan me-1"></i> เข้าสู่ระบบ e-Claim (ThaiD)');
