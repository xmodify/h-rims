    ...
    <div class="row justify-content-center mt-3 mb-4">
        <div class="col-md-8">
            <div class="card dash-card accent-9">
                <div class="card-body">
                    <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('import/stm_bmt_kidney_save') }}" method="POST" enctype="multipart/form-data" class="m-0">
                        @csrf
                        <div class="text-center mb-3">
                            <h6 class="fw-bold text-dark"><i class="bi bi-file-earmark-excel me-2 text-success"></i> นำเข้าไฟล์ STM BMT (Excel Only)</h6>
    ...
    <div class="page-header-box">
        <div>
            <h5 class="text-dark mb-0 fw-bold">
                ...
                ข้อมูล Statement ขสมก. (BMT) [ฟอกไต HD]
            </h5>
            <div class="text-muted small mt-1">ปีงบประมาณประจำปัจจุบัน: {{ $budget_year }}</div>
            <div class="mt-2">
                <a href="{{ url('/import/stm_bmt_kidneydetail') }}" class="btn btn-primary btn-sm rounded-pill px-3">
    ...
    <div class="card dash-card border-top-0">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="stm_bmt_kidney" class="table table-modern w-100">
                    <thead>
                        <tr>
            ...
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="text-muted small" id="db_subtitle">ยอดชดเชยสุทธิรายเดือน Statement ขสมก. (BMT) [ฟอกไต HD]</div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('btnSaveReceipt')
                .addEventListener('click', function () {
                    ...
                    fetch("{{ url('import/stm_bmt_kidney_updateReceipt') }}", {
                        method: "POST",
                        headers: {
        ...
        function simulateProcess(event) {
            ...
            if (fileInput.files.length > 5) {
                Swal.fire({
                    title: 'แจ้งเตือน',
                    text: 'เลือกไฟล์ได้สูงสุดไม่เกิน 5 ไฟล์',
                    icon: 'error',
                    confirmButtonText: 'ปิด',
                    confirmButtonColor: '#673ab7',
                    customClass: {
    ...
    <script>
        $(document).ready(function () {
            ...
            var stmTable = $('#stm_bmt_kidney').DataTable({
                ordering: false,
                dom: '<"row mb-3"' +
                        '<"col-md-6"l>' +
                ...
                buttons: [
                {
                    ...
                    title: 'ข้อมูล Statement เบิกจ่ายตรง ขสมก. BMT [ฟอกไต]'
                }
                ],
                language: {
                    search: "ค้นหา:",
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex, rowData, counter) {
                    if (settings.sTableId !== 'stm_bmt_kidney') return true;
            ...
            function loadChartData() {
                ...
                $('#db_subtitle').text(`ยอดชดเชยสุทธิรายงวด Statement ขสมก. (BMT) [ฟอกไต HD] ปีงบประมาณ: ${budgetYearText}`);

                $('#chart_container').addClass('d-none');
                $('#loading_spinner').removeClass('d-none');

                $.ajax({
                    url: "{{ route('import.stm_bmt_kidney.chart-data') }}",
                    method: "GET",
                    data: {
