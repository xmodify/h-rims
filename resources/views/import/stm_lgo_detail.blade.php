...
<div class="container-fluid px-lg-4">
    ...
    <div class="page-header-box mt-3 mb-4">
        <div>
            <h5 class="text-dark mb-0 fw-bold">
                ...
                รายละเอียด Statement เบิกจ่ายตรง อปท.LGO [OP-IP]
            </h5>
    ...
    <div class="card dash-card border-top-0">
        ...
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="stm_lgo_list_ip" class="table table-modern w-100">
                    <thead>
                        <tr>
                            <th class="text-center">Filename</th> 
                            <th class="text-center">REP</th> 
...
<script>
    $(document).ready(function () {
      ...
      $('#stm_lgo_list').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('stm_lgo_detail') }}",
            data: function (d) {
                d.start_date = $('#start_date').val();
                d.end_date = $('#end_date').val();
                d.type = 'opd'; // Identify as OPD request
            }
        },
        columns: [
            { data: 'stm_filename', name: 'stm_filename', className: 'small fw-bold text-dark' },
            { 
                data: 'repno', 
                name: 'repno', 
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    ...
                    window.location.href = "{{ route('stm_lgo_detail') }}?export=excel&type=opd&start_date=" + start + "&end_date=" + end;
                }
            }
        ],
        language: {
      ...
      $('#stm_lgo_list_ip').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                d.start_date = $('#start_date').val();
                d.end_date = $('#end_date').val();
                d.type = 'ipd'; // Identify as IPD request
            }
        },
        columns: [
            { data: 'stm_filename', name: 'stm_filename', className: 'small fw-bold text-dark' },
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    var start = $('#start_date').val();
                    var end = $('#end_date').val();
                    window.location.href = "{{ route('stm_lgo_detail') }}?export=excel&type=ipd&start_date=" + start + "&end_date=" + end;
                }
            }
        ],
        language: {
