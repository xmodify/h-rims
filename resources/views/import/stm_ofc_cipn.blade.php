...
<div class="container-fluid px-lg-4">
    ...
    <div class="row justify-content-center mt-3 mb-4">
        <div class="col-md-8">
            <div class="card dash-card accent-9">
                <div class="card-body">
                    <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('import/stm_ofc_cipn_save') }}" method="POST" enctype="multipart/form-data" class="m-0">
                        @csrf
                        <div class="text-center mb-3">
                            <h6 class="fw-bold text-dark"><i class="bi bi-file-earmark-zip me-2 text-primary"></i> นำเข้าไฟล์ STM CIPN (.zip)</h6>
                            <p class="text-muted small">เลือกไฟล์ .zip ที่ได้รับจากระบบ CIPN</p>
                        </div>
    ...
    <div class="page-header-box">
        <div>
            ...
            <div class="mt-2">
                <a href="{{ url('/import/stm_ofc_cipndetail') }}" class="btn btn-primary btn-sm rounded-pill px-3">
    ...
    <div class="card dash-card border-top-0">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="stm_ofc_cipn" class="table table-modern w-100">
                    <thead>
                        <tr>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow-lg rounded-3 border-0 py-1.5">
                                        ...
                                        <li>
                                            <a href="javascript:void(0);" 
                                               ...
                                               data-type="stm_ofc_cipn"
                                               data-round="{{ $row->round_no }}"
                                               data-filename="{{ $row->stm_filename }}"
                                               data-count="{{ $row->count_no ?? 0 }}"
                                               data-amount="{{ number_format($row->gtotal ?? 0, 2) }}"
                                               data-receive="{{ $row->receive_no }}"
                                               data-date="{{ $row->receipt_date }}">
            ...
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="text-muted small" id="db_subtitle">ยอดชดเชยสุทธิรายเดือน Statement สวัสดิการข้าราชการ CIPN</div>
                    </div>
                </div>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('btnSaveReceipt')
                .addEventListener('click', function () {
                    ...
                    fetch("{{ url('import/stm_ofc_cipn_updateReceipt') }}", {
                        method: "POST",
                        headers: {
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            var stmTable = $('#stm_ofc_cipn').DataTable({
                ...
                buttons: [
                {
                    ...
                    title: 'ข้อมูล Statement สวัสดิการข้าราชการ CIPN'
                }
                ],
                language: {
                    search: "ค้นหา:",
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex, rowData, counter) {
                    if (settings.sTableId !== 'stm_ofc_cipn') return true;
            ...
            function loadChartData() {
                ...
                $('#db_subtitle').text(`ยอดชดเชยสุทธิรายงวด Statement สวัสดิการข้าราชการ CIPN ปีงบประมาณ: ${budgetYearText}`);

                $('#chart_container').addClass('d-none');
                $('#loading_spinner').removeClass('d-none');

                $.ajax({
                    url: "{{ route('import.stm_ofc_cipn.chart-data') }}",
                    method: "GET",
                    data: {
