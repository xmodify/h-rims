...
<div class="container-fluid px-lg-4">
    ...
    <div class="page-header-box">
        <div>
            ...
            <div class="mt-2">
                <a href="{{ url('/import/stm_ofc_csopdetail') }}" class="btn btn-primary btn-sm rounded-pill px-3">
                    <i class="bi bi-file-earmark-text me-1"></i> รายละเอียด
                </a>
                <button type="button" class="btn btn-info btn-sm rounded-pill px-3 text-white shadow-sm" data-bs-toggle="modal" data-bs-target="#chartModal" id="btnShowChartCsop" data-sys-type="csop">
                    <i class="bi bi-bar-chart-fill me-1"></i> กราฟสรุปรายเดือน CSOP
                </button>
                <button type="button" class="btn btn-warning btn-sm rounded-pill px-3 text-white shadow-sm" data-bs-toggle="modal" data-bs-target="#chartModal" id="btnShowChartKidney" data-sys-type="kidney">
                    <i class="bi bi-bar-chart-fill me-1"></i> กราฟสรุปฟอกไต
                </button>
            </div>
        </div>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow-lg rounded-3 border-0 py-1.5">
                                        ...
                                        <li>
                                            <a href="javascript:void(0);" 
                                               ...
                                               data-type="stm_ofc_csop"
                                               data-round="{{ $row->round_no }}"
                                               data-filename="{{ $row->stm_filename }}"
                                               data-count="{{ $row->count_no ?? 0 }}"
                                               data-amount="{{ number_format($row->amount ?? 0, 2) }}"
                                               data-receive="{{ $row->receive_no }}"
                                               data-date="{{ $row->receipt_date }}">
                                                <i class="bi bi-file-earmark-text-fill text-primary me-2 fs-6"></i>
            ...
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="text-muted small" id="db_subtitle">ยอดชดเชยสุทธิรายเดือน Statement สวัสดิการข้าราชการ CSOP</div>
                    </div>
                </div>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('btnSaveReceipt')
                .addEventListener('click', function () {
                    ...
                    fetch("{{ url('import/stm_ofc_csop_updateReceipt') }}", {
                        method: "POST",
                        headers: {
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            var stmTable = $('#stm_ofc_csop').DataTable({
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex, rowData, counter) {
                    if (settings.sTableId !== 'stm_ofc_csop') return true;
            ...
            function initChartEvent() {
                $('#btnShowChartCsop').on('click', function() {
                    currentSysType = 'csop';
                });

                $('#btnShowChartKidney').on('click', function() {
                    currentSysType = 'kidney';
                });
            ...
            function loadChartData() {
                ...
                const sysName = currentSysType === 'kidney' ? 'ฟอกไต OFC' : 'สวัสดิการข้าราชการ CSOP';

                $('#db_title').text(`Dashboard (${sysName})`);
                $('#db_subtitle').text(`ยอดชดเชยสุทธิรายงวด Statement ${sysName} ปีงบประมาณ: ${budgetYearText}`);

                $('#chart_container').addClass('d-none');
                $('#loading_spinner').removeClass('d-none');

                $.ajax({
                    url: "{{ route('import.stm_ofc_csop.chart-data') }}",
                    method: "GET",
                    data: {
                        budget_year: budgetYear,
                        sys_type: currentSysType
                    },
