                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow-lg rounded-3 border-0 py-1.5">
                                        ...
                                        @if(Auth::user()->status == 'admin' || Auth::user()->allow_receipt == 'Y')
                                            <li>
                                                <a href="javascript:void(0);" 
                                                   class="dropdown-item d-flex align-items-center {{ $hasReceipt ? 'btn-edit-receipt text-warning' : 'btn-new-receipt text-success' }}"
                                                   data-round="{{ $row->round_no }}"
                                                   data-receive="{{ $effectiveReceiptNo }}"
                                                   data-date="{{ $effectiveReceiptDate }}"
                                                   data-bs-toggle="modal"
                                                   data-bs-target="#receiptModal">
                                                    <i class="bi {{ $hasReceipt ? 'bi-pencil-square text-warning' : 'bi-receipt text-success' }} me-2 fs-6"></i>
                                                    <span class="fw-semibold">{{ $hasReceipt ? 'แก้ไขเลขที่ใบเสร็จ' : 'ออกใบเสร็จรับเงิน' }}</span>
                                                </a>
                                            </li>
                                        @endif
                                        ...
                                        <li>
                                            <a href="javascript:void(0);" 
                                               ...
                                               data-amount="{{ number_format($stmTotal, 2) }}"
                                               data-receive="{{ $effectiveReceiptNo }}"
                                               data-date="{{ $effectiveReceiptDate }}">
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res && res.connected) {
                        ...
                        $('#btnEclaimLoginPopup').html('<i class="bi bi-arrow-repeat me-1"></i> เข้าสู่ระบบใหม่ / เปลี่ยนบัญชี ThaiD');
                        ...
                        $('#btnEclaimLoginPopup').html('<i class="bi bi-qr-code-scan me-1"></i> เข้าสู่ระบบ e-Claim (ThaiD)');
