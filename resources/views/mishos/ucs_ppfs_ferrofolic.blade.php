...
#claim-tab.active {
    ...
    <div class="page-header-box mt-2 mb-3 d-flex justify-content-between align-items-center">
        <div>
            <h4 class="text-primary mb-0 fw-bold">
                ...
                รายชื่อผู้รับบริการยาเสริมธาตุเหล็ก (Ferrofolic & Iron)
            </h4>
        </div>
    ...
    <x-import_hub_modal 
        ...
        :has-edc="false" 
        :has-fdh="true"
        claim-title="MIS Hospital - รายชื่อผู้รับบริการเสริมธาตุเหล็กและกรดโฟลิก" 
    />
...
<script>
    ...
    $(document).ready(function () {
        ...
        window.initDataTables = function() {
            const tableIds = ['#t_search_tablet', '#t_search_syrup', '#t_search_syrup_6_12m', '#t_search_syrup_1_5y', '#t_search_syrup_preschool', '#t_search_syrup_school', '#t_claim'];
                ...
                buttons: [
                    {
                      ...
                      title: 'รายชื่อผู้รับบริการเสริมธาตุเหล็กและกรดโฟลิก วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}'
                    }
                ],
                language: {
                    search: "ค้นหา:",
    ...
    // Function ส่งออก 16 แฟ้ม FDH ที่เลือก
    function exportSelectedF16FDH(claimCode) {
        // 1. ดึง Checkbox ที่ถูกเลือกและแสดงอยู่บนหน้าจอ (Visible ใน Tab/Sub-tab ปัจจุบัน)
        let checkedElements = Array.from(document.querySelectorAll('.f16-select-item:checked')).filter(cb => cb.offsetParent !== null);
        
        // 2. ถ้าไม่มีในแท็บที่กำลังแสดงอยู่ ให้ดูรายการที่เลือกทั้งหมด
        if (checkedElements.length === 0) {
            checkedElements = Array.from(document.querySelectorAll('.f16-select-item:checked'));
        }

        const selectedVns = [...new Set(checkedElements.map(cb => cb.value).filter(Boolean))];
        ...
        openF16FdhExportModal({
            vns: selectedVns,
            claimCode: claimCode || 'UCS_PPFS_FERROFOLIC',
            claimTitle: 'บริการเสริมธาตุเหล็กและกรดโฟลิก (Ferrofolic & Iron)',
            isIp: false
        });
    }

    function exportSingleVnFdh() {
        if (!currentModalVn) return;
        $('#detailsModal').modal('hide');
        openF16FdhExportModal({
            vns: [currentModalVn],
            ans: [currentModalVn],
            claimCode: 'UCS_PPFS_FERROFOLIC',
            claimTitle: 'บริการเสริมธาตุเหล็กและกรดโฟลิก (Ferrofolic & Iron)',
            isIp: false
        });
    }
    ...
    function showDetails(vn) {
        ...
        .done(function(data) {
            ...
            function makeCellHtml(isValid, epDone, warn) {
                if (!isValid) {
                    return `<button class="btn btn-sm btn-outline-danger px-2 py-1 border-2 d-flex align-items-center justify-content-center" style="font-size:0.7rem; height: 26px; min-height: 26px; margin: 0 auto;" onclick="showDetails('${vn}')" title="ไม่ผ่านเงื่อนไข 16 แฟ้ม | คลิกดูรายละเอียด"><i class="bi bi-eye-fill"></i></button>`;
                } else if (warn) {
                    return `<button class="btn btn-sm btn-outline-warning px-2 py-1 border-2 d-flex align-items-center justify-content-center" style="font-size:0.7rem; height: 26px; min-height: 26px; margin: 0 auto;" onclick="showDetails('${vn}')" title="มีคำเตือน 16 แฟ้ม | คลิกดูรายละเอียด"><i class="bi bi-eye-fill"></i></button>`;
                } else if (epDone) {
                    return `<button class="btn btn-sm btn-outline-success px-2 py-1 border-2 d-flex align-items-center justify-content-center" style="font-size:0.7rem; height: 26px; min-height: 26px; margin: 0 auto;" onclick="showDetails('${vn}')" title="ผ่านเงื่อนไข 16 แฟ้ม + ปิดสิทธิแล้ว | ดูรายละเอียด"><i class="bi bi-eye-fill"></i></button>`;
                } else {
                    return `<button class="btn btn-sm btn-outline-warning px-2 py-1 border-2 d-flex align-items-center justify-content-center" style="font-size:0.7rem; height: 26px; min-height: 26px; margin: 0 auto;" onclick="showDetails('${vn}')" title="16 แฟ้มครบ แต่ยังไม่ปิดสิทธิ สปสช. | คลิกดูรายละเอียด"><i class="bi bi-eye-fill"></i></button>`;
                }
            }

            const dataOrder = !v.is_valid ? '0' : (isEndpointDone && !hasWarnings ? '2' : '1');

            const searchRow = document.getElementById(`td-status-search-${vn}`) || document.getElementById(`td-status-claim-${vn}`);
            if (searchRow) {
                searchRow.innerHTML = makeCellHtml(v.is_valid, isEndpointDone, hasWarnings);
                searchRow.setAttribute('data-order', dataOrder);
                ['#t_search_tablet', '#t_search_syrup', '#t_search_syrup_6_12m', '#t_search_syrup_1_5y', '#t_search_syrup_preschool', '#t_search_syrup_school', '#t_claim'].forEach(function(tblId) {
                    if ($.fn.DataTable.isDataTable(tblId)) {
                        $(tblId).DataTable().cell(searchRow).invalidate().draw(false);
                    }
                });
            }
            ...
            const ucMoney = visit.uc_money !== undefined ? visit.uc_money : (parseFloat(visit.income || 0) - parseFloat(visit.rcpt_money || 0));
                ...
                <div class="card border-0 bg-light-soft h-100">
                  <div class="card-body py-2 px-3">
                    ...
                    <table class="table table-sm table-borderless mb-0 small compact-info-table">
                      ...
                      <tr><th class="text-muted">ยอดเรียกเก็บ</th><td>${parseFloat(ucMoney).toFixed(2)} บาท</td></tr>
        ...
        .fail(function() {
            body.innerHTML = '<div class="alert alert-danger text-center py-4">ไม่สามารถโหลดข้อมูลรายละเอียดการรับบริการได้</div>';
        });
    }
