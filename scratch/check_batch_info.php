...
$data = $res->json();
$datas = $data['datas'] ?? [];
echo "Datas count: " . count($datas) . "\n";
foreach ($datas as $d) {
    echo "Batch: {$d['batchNo']}, Round: {$d['refDocNo']}, From: " . ($d['fromSystem'] ?? 'none') . ", RecId: " . ($d['recId'] ?? 'none') . ", LinkConfig: " . ($d['linkConfigId'] ?? 'none') . ", mophId: " . ($d['mophId'] ?? 'none') . "\n";
}
