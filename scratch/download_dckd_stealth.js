...
(async () => {
    ...
    await page.goto(dmisUrl, { waitUntil: 'domcontentloaded' });

    console.log("3. กำลังรอปุ่ม Export Excel (รอได้ถึง 90 วินาที)...");
    const exportBtn = await page.waitForSelector('mat-chip:has-text("Export Excel"), button:has-text("Export Excel"), :text("Export Excel")', { timeout: 90000 }).catch(() => null);

    await page.screenshot({ path: path.join(__dirname, 'dckd_loaded.png') });
    console.log("Screenshot saved to scratch/dckd_loaded.png");

    if (exportBtn) {
        console.log("🎉 พบปุ่ม Export Excel! กำลังเริ่มดาวน์โหลด...");
        ...
        console.log("❌ ไม่พบปุ่ม Export Excel ภายใน 90 วินาที");
    }
